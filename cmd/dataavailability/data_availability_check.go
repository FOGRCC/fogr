...
import (
	...
	"github.com/FOGRCC/fogr/util/metricsutil"
...
const metricBaseOldHash = "fogr/das/dataavailability/oldhash/"
const metricBaseNewHash = "fogr/das/dataavailability/oldhash/"
...
type DataAvailabilityCheck struct {
	...
	urlToReaderMap map[string]fogstate.DataAvailabilityReader
...
func newDataAvailabilityCheck(ctx context.Context, dataAvailabilityCheckConfig *DataAvailabilityCheckConfig) (*DataAvailabilityCheck, error) {
	...
	urlToReaderMap := make(map[string]fogstate.DataAvailabilityReader, len(onlineUrls))
...
func (d *DataAvailabilityCheck) checkDataAvailability(ctx context.Context, deliveredLog types.Log, metricBase string) (bool, error) {
	...
	cert, err := fogstate.DeserializeDASCertFrom(bytes.NewReader(data))
	if err != nil {
		return true, err
	}
