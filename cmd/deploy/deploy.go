...
func main() {
	...
	l2ChainIdUint := flag.Uint64("l2chainid", params.FOGDevTestChainConfig().ChainID.Uint64(), "L2 chain ID")
	...
	if *prod {
		if *l2ChainIdUint == params.FOGDevTestChainConfig().ChainID.Uint64() {
	...
	deployPtr, err := fognode.DeployOnL1(
		ctx,
		l1client,
		l1TransactionOpts,
		sequencerAddress,
		*authorizevalidators,
		...
		fognode.GenerateRollupConfig(*prod, moduleRoot, ownerAddress, l2ChainId, loserEscrowAddress),
	)
	if err != nil {
		flag.Usage()
