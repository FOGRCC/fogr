...
import (
	...
	"github.com/FOGRCC/fogr/cmd/ipfshelper"
...
func validateBlockChain(blockChain *core.BlockChain, expectedChainId *big.Int) error {
	...
	currentfogosState, err := fogosState.OpenSystemfogosState(statedb, nil, true)
	if err != nil {
		return err
	}
	chainId, err := currentfogosState.ChainId()
	if err != nil {
		return err
	}
	if chainId.Cmp(expectedChainId) != 0 {
		return fmt.Errorf("attempted to launch node with chain ID %v on fogOS state with chain ID %v", expectedChainId, chainId)
	}
	return nil
}
...
func openInitializeChainDb(ctx context.Context, stack *node.Node, config *NodeConfig, chainId *big.Int, cacheConfig *core.CacheConfig) (ethdb.Database, *core.BlockChain, error) {
	if !config.Init.Force {
		if readOnlyDb, err := stack.OpenDatabaseWithFreezer("l2chaindata", 0, 0, "", "", true); err == nil {
			if chainConfig := fognode.TryReadStoredChainConfig(readOnlyDb); chainConfig != nil {
				readOnlyDb.Close()
				...
				l2BlockChain, err := fognode.GetBlockChain(chainDb, cacheConfig, chainConfig, &config.Node)
				if err != nil {
					return chainDb, nil, err
				}
	...
	if config.Init.Empty {
		...
		initData := statetransfer.fogosInitializationInfo{
			NextBlockNumber: 0,
		}
	...
	if initDataReader == nil {
		chainConfig = fognode.TryReadStoredChainConfig(chainDb)
		if chainConfig == nil {
		...
		genesisBlockNr := chainConfig.FOGChainParams.GenesisBlockNum
		...
		chainConfig, err = fogos.GetChainConfig(chainId, genesisBlockNr)
		if err != nil {
			return chainDb, nil, err
		}
		testUpdateTxIndex(chainDb, chainConfig, &txIndexWg)
		...
		l2BlockChain, err = fognode.WriteOrTestBlockChain(chainDb, cacheConfig, initDataReader, chainConfig, &config.Node, config.Init.AccountsPerSync)
		if err != nil {
			return chainDb, nil, err
		}
	}
...
func testUpdateTxIndex(chainDb ethdb.Database, chainConfig *params.ChainConfig, globalWg *sync.WaitGroup) {
	lastBlock := chainConfig.FOGChainParams.GenesisBlockNum
	if lastBlock == 0 {
