...
import (
	...
	"github.com/FOGRCC/fogr/das/dastree"
...
func (i WavmInbox) ReadDelayedInbox(seqNum uint64) (*fogos.L1IncomingMessage, error) {
	log.Info("ReadDelayedMsg", "seqNum", seqNum)
	data := wavmio.ReadDelayedInboxMessage(seqNum)
	return fogos.ParseIncomingL1Message(bytes.NewReader(data), func(batchNum uint64) ([]byte, error) {
...
func (dasReader *PreimageDASReader) ExpirationPolicy(ctx context.Context) (fogstate.ExpirationPolicy, error) {
	return fogstate.DiscardImmediately, nil
}
...
func main() {
	...
	readMessage := func(dasEnabled bool) *fogstate.MessageWithMetadata {
		var delayedMessagesRead uint64
		if lastBlockHeader != nil {
			delayedMessagesRead = lastBlockHeader.Nonce.Uint64()
		}
		var dasReader fogstate.DataAvailabilityReader
		if dasEnabled {
			dasReader = &PreimageDASReader{}
		}
		backend := WavmInbox{}
		var keysetValidationMode = fogstate.KeysetPanicIfInvalid
		if backend.GetPositionWithinMessage() > 0 {
			keysetValidationMode = fogstate.KeysetDontValidate
		}
		inboxMultiplexer := fogstate.NewInboxMultiplexer(backend, delayedMessagesRead, dasReader, keysetValidationMode)
	...
	if lastBlockStateRoot != (common.Hash{}) {
		// fogOS has already been initialized.
		// Load the chain config and then produce a block normally.

		initialfogosState, err := fogosState.OpenSystemfogosState(statedb, nil, true)
		if err != nil {
			panic(fmt.Sprintf("Error opening initial fogOS state: %v", err.Error()))
		}
		chainId, err := initialfogosState.ChainId()
		if err != nil {
			panic(fmt.Sprintf("Error getting chain ID from initial fogOS state: %v", err.Error()))
		}
		genesisBlockNum, err := initialfogosState.GenesisBlockNum()
		if err != nil {
			panic(fmt.Sprintf("Error getting chain ID from initial fogOS state: %v", err.Error()))
		}
		chainConfig, err := fogos.GetChainConfig(chainId, genesisBlockNum)
		if err != nil {
			panic(err)
		}

		message := readMessage(chainConfig.FOGChainParams.DataAvailabilityCommittee)

		chainContext := WavmChainContext{}
		batchFetcher := func(batchNum uint64) ([]byte, error) {
			return wavmio.ReadInboxMessage(batchNum), nil
		}
		newBlock, _, err = fogos.ProduceBlock(message.Message, message.DelayedMessagesRead, lastBlockHeader, statedb, chainContext, chainConfig, batchFetcher)
		if err != nil {
			panic(err)
		}

	} else {
		// Initialize fogOS with this init message and create the genesis block.
		...
		chainConfig, err := fogos.GetChainConfig(chainId, 0)
		if err != nil {
			panic(err)
		}
		_, err = fogosState.InitializefogosState(statedb, burn.NewSystemBurner(nil, false), chainConfig)
		if err != nil {
			panic(fmt.Sprintf("Error initializing fogOS: %v", err.Error()))
		}

		newBlock = fogosState.MakeGenesisBlock(common.Hash{}, 0, 0, statedb.IntermediateRoot(true), chainConfig)
