...
var (
	sourcesConnectedGauge    = metrics.NewRegisteredGauge("fogr/feed/sources/connected", nil)
	sourcesDisconnectedGauge = metrics.NewRegisteredGauge("fogr/feed/sources/disconnected", nil)
)
...
type BroadcastClient struct {
	...
	websocketUrl string
	nextSeqNum   fogutil.MessageIndex
	...
	retrying                        bool
	shuttingDown                    bool
	confirmedSequenceNumberListener chan fogutil.MessageIndex
...
func NewBroadcastClient(
	...
	websocketUrl string,
	chainId uint64,
	currentMessageCount fogutil.MessageIndex,
	txStreamer TransactionStreamerInterface,
	confirmedSequencerNumberListener chan fogutil.MessageIndex,
	fatalErrChan chan error,
...
func (bc *BroadcastClient) connect(ctx context.Context, nextSeqNum fogutil.MessageIndex) (io.Reader, error) {
	...
	log.Info("connecting to FOGR inbox message broadcaster", "url", bc.websocketUrl)
