...
func testReceiveMessages(t *testing.T, clientCompression bool, serverCompression bool, serverRequire bool, expectNoMessagesReceived bool) {
	...
	go func() {
		for i := 0; i < messageCount; i++ {
			Require(t, b.BroadcastSingle(fogstate.TestMessageWithMetadataAndRequestId, fogutil.MessageIndex(i)))
		}
	}()
...
func newTestBroadcastClient(config Config, listenerAddress net.Addr, chainId uint64, currentMessageCount fogutil.MessageIndex, txStreamer TransactionStreamerInterface, confirmedSequenceNumberListener chan fogutil.MessageIndex, feedErrChan chan error, validAddr *common.Address) (*BroadcastClient, error) {
...
func TestServerClientDisconnect(t *testing.T) {
	...
	Require(t, b.BroadcastSingle(fogstate.EmptyTestMessageWithMetadata, 0))
...
func TestBroadcastClientConfirmedMessage(t *testing.T) {
	...
	confirmedSequenceNumberListener := make(chan fogutil.MessageIndex, 10)
	ts := NewDummyTransactionStreamer(chainId, nil)
	broadcastClient, err := newTestBroadcastClient(
		DefaultTestConfig,
		b.ListenerAddr(),
	...
	confirmNumber := fogutil.MessageIndex(42)
	b.Confirm(42)
...
func TestBroadcasterSendsCachedMessagesOnClientConnect(t *testing.T) {
	...
	*/
	...
	Require(t, b.BroadcastSingle(fogstate.EmptyTestMessageWithMetadata, 0))
	Require(t, b.BroadcastSingle(fogstate.EmptyTestMessageWithMetadata, 1))
