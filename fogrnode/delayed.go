...
func NewDelayedBridge(client fogutil.L1Interface, addr common.Address, fromBlock uint64) (*DelayedBridge, error) {
...
func (m *DelayedInboxMessage) AfterInboxAcc() common.Hash {
	hash := crypto.Keccak256(
		...
		fogmath.UintToBytes(m.Message.Header.BlockNumber),
		fogmath.UintToBytes(m.Message.Header.Timestamp),
...
func (b *DelayedBridge) LookupMessagesInRange(ctx context.Context, from, to *big.Int, batchFetcher fogos.FallibleBatchFetcher) ([]*DelayedInboxMessage, error) {
...
func (b *DelayedBridge) logsToDeliveredMessages(ctx context.Context, logs []types.Log, batchFetcher fogos.FallibleBatchFetcher) ([]*DelayedInboxMessage, error) {
	if len(logs) == 0 {
		return nil, nil
	}
...
func (b *DelayedBridge) parseMessage(ctx context.Context, ethLog types.Log) (*big.Int, []byte, error) {
	...
	if ethLog.Topics[0] == inboxMessageDeliveredID {
		...
		data, err := fogutil.GetLogEmitterTxData(ctx, b.client, ethLog)
		if err != nil {
			return nil, nil, err
		}
