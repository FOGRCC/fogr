...
import (
	...
	"github.com/FOGRCC/fogr/broadcaster"
	...
	"github.com/FOGRCC/fogr/staker"
	"github.com/FOGRCC/fogr/util/sharedmetrics"
	"github.com/FOGRCC/fogr/util/stopwaiter"
)
...
func (s *TransactionStreamer) ReorgTo(count fogutil.MessageIndex) error {
	return s.ReorgToAndEndBatch(s.db.NewBatch(), count)
}

func (s *TransactionStreamer) ReorgToAndEndBatch(batch ethdb.Batch, count fogutil.MessageIndex) error {
...
func (s *TransactionStreamer) reorgToInternal(batch ethdb.Batch, count fogutil.MessageIndex) ([]*fogstate.MessageWithMetadata, error) {
	if count == 0 {
		return nil, errors.New("cannot reorg out init message")
	}
	var oldMessages []*fogstate.MessageWithMetadata
	if s.reorgSequencing {
		...
		config := s.config()
		maxResequenceMsgCount := count + fogutil.MessageIndex(config.MaxReorgResequenceDepth)
...
func setMessageCount(batch ethdb.KeyValueWriter, count fogutil.MessageIndex) error {
...
func (s *TransactionStreamer) GetMessage(seqNum fogutil.MessageIndex) (*fogstate.MessageWithMetadata, error) {
	key := dbKey(messagePrefix, uint64(seqNum))
	data, err := s.db.Get(key)
	if err != nil {
		return nil, err
	}
	var message fogstate.MessageWithMetadata
...
// Note: if changed to acquire the mutex, some internal users may need to be updated to a non-locking version.
func (s *TransactionStreamer) GetMessageCount() (fogutil.MessageIndex, error) {
	...
	return fogutil.MessageIndex(pos), nil
}

func (s *TransactionStreamer) AddMessages(pos fogutil.MessageIndex, messagesAreConfirmed bool, messages []fogstate.MessageWithMetadata) error {
...
func (s *TransactionStreamer) AddBroadcastMessages(feedMessages []*broadcaster.BroadcastFeedMessage) error {
	...
	var messages []fogstate.MessageWithMetadata
	...
	broadcastStartPos += fogutil.MessageIndex(dups)
	if oldMsg != nil {
	...
	if len(s.broadcasterQueuedMessages) == 0 || (feedReorg && !s.broadcasterQueuedMessagesActiveReorg) {
		...
		broadcasterQueuedMessagesPos := fogutil.MessageIndex(atomic.LoadUint64(&s.broadcasterQueuedMessagesPos))
		if broadcasterQueuedMessagesPos >= broadcastStartPos {
			// Feed messages older than cache
			s.broadcasterQueuedMessages = messages
			atomic.StoreUint64(&s.broadcasterQueuedMessagesPos, uint64(broadcastStartPos))
			s.broadcasterQueuedMessagesActiveReorg = feedReorg
		} else if broadcasterQueuedMessagesPos+fogutil.MessageIndex(len(s.broadcasterQueuedMessages)) == broadcastStartPos {
			...
			if len(s.broadcasterQueuedMessages) > 0 {
				log.Warn(
					...
					"expectedNextPos", broadcasterQueuedMessagesPos+fogutil.MessageIndex(len(s.broadcasterQueuedMessages)),
...
func (s *TransactionStreamer) AddFakeInitMessage() error {
	return s.AddMessages(0, false, []fogstate.MessageWithMetadata{{
		Message: &fogos.L1IncomingMessage{
			Header: &fogos.L1IncomingMessageHeader{
				Kind:      fogos.L1MessageType_Initialize,
...
// Used in redis tests
func (s *TransactionStreamer) GetMessageCountSync() (fogutil.MessageIndex, error) {
	s.insertionMutex.Lock()
	defer s.insertionMutex.Unlock()
...
func (s *TransactionStreamer) AddMessagesAndEndBatch(pos fogutil.MessageIndex, messagesAreConfirmed bool, messages []fogstate.MessageWithMetadata, batch ethdb.Batch) error {
...
func (s *TransactionStreamer) getPrevPrevDelayedRead(pos fogutil.MessageIndex) (uint64, error) {
...
func (s *TransactionStreamer) countDuplicateMessages(
	pos fogutil.MessageIndex,
	messages []fogstate.MessageWithMetadata,
	batch *ethdb.Batch,
) (int, bool, *fogstate.MessageWithMetadata, error) {
	curMsg := 0
	for {
		...
		if !bytes.Equal(haveMessage, wantMessage) {
			...
			var dbMessageParsed fogstate.MessageWithMetadata
			...
			if err != nil {
				...
				if nextMessage.Message != nil {
					if dbMessageParsed.Message.BatchGasCost == nil || nextMessage.Message.BatchGasCost == nil {
						...
						nextMessageCopy.Message = new(fogos.L1IncomingMessage)
...
func (s *TransactionStreamer) logReorg(pos fogutil.MessageIndex, dbMsg *fogstate.MessageWithMetadata, newMsg *fogstate.MessageWithMetadata, confirmed bool) {
	sendLog := confirmed
...
func (s *TransactionStreamer) addMessagesAndEndBatchImpl(messageStartPos fogutil.MessageIndex, messagesAreConfirmed bool, messages []fogstate.MessageWithMetadata, batch ethdb.Batch) error {
	var confirmedReorg bool
	var oldMsg *fogstate.MessageWithMetadata
	var lastDelayedRead uint64
	var hasNewConfirmedMessages bool

	messagesAfterPos := messageStartPos + fogutil.MessageIndex(len(messages))
	broadcastStartPos := fogutil.MessageIndex(atomic.LoadUint64(&s.broadcasterQueuedMessagesPos))
	...
	if messagesAreConfirmed {
		...
		if duplicates > 0 {
			...
			messageStartPos += fogutil.MessageIndex(duplicates)
		}
		if len(messages) > 0 {
	...
	for i, msg := range messages {
		msgPos := messageStartPos + fogutil.MessageIndex(i)
...
func messageFromTxes(header *fogos.L1IncomingMessageHeader, txes types.Transactions, txErrors []error) (*fogos.L1IncomingMessage, error) {
	var l2Message []byte
	if len(txes) == 1 && txErrors[0] == nil {
		...
		l2Message = append(l2Message, fogos.L2MessageKind_SignedTx)
		l2Message = append(l2Message, txBytes...)
	} else {
		l2Message = append(l2Message, fogos.L2MessageKind_Batch)
		sizeBuf := make([]byte, 8)
		for i, tx := range txes {
			if txErrors[i] != nil {
				continue
	...
	return &fogos.L1IncomingMessage{
		Header: header,
		L2msg:  l2Message,
	}, nil
}
...
func (s *TransactionStreamer) resequenceReorgedMessages(messages []*fogstate.MessageWithMetadata, lastDelayedSeqNum uint64) {
	for _, msg := range messages {
		...
		if header.RequestId != nil {
			...
			// The context is only necessary for batch fetching,
			// which will no longer be necessary with the batch posting report refactor.
			err := s.sequenceDelayedMessagesWithInsertionMutex(context.TODO(), []*fogos.L1IncomingMessage{msg.Message}, delayedSeqNum)
			if err != nil {
		...
		if header.Kind != fogos.L1MessageType_L2Message || header.Poster != l1pricing.BatchPosterAddress {
		...
		hooks := fogos.NoopSequencingHooks()
...
func (s *TransactionStreamer) SequenceTransactions(header *fogos.L1IncomingMessageHeader, txes types.Transactions, hooks *fogos.SequencingHooks) (*types.Block, error) {
	s.insertionMutex.Lock()
	defer s.insertionMutex.Unlock()
	return s.sequenceTransactionsWithInsertionMutex(header, txes, hooks)
}

func (s *TransactionStreamer) sequenceTransactionsWithInsertionMutex(header *fogos.L1IncomingMessageHeader, txes types.Transactions, hooks *fogos.SequencingHooks) (*types.Block, error) {
	...
	startTime := time.Now()
	block, receipts, err := fogos.ProduceBlockAdvanced(
		header,
		txes,
		delayedMessagesRead,
		lastBlockHeader,
	...
	msgWithMeta := fogstate.MessageWithMetadata{
		Message:             msg,
	...
	if err := s.writeMessages(pos, []fogstate.MessageWithMetadata{msgWithMeta}, nil); err != nil {
		return nil, err
	}
...
func (s *TransactionStreamer) SequenceDelayedMessages(ctx context.Context, messages []*fogos.L1IncomingMessage, firstDelayedSeqNum uint64) error {
	s.insertionMutex.Lock()
	defer s.insertionMutex.Unlock()
	return s.sequenceDelayedMessagesWithInsertionMutex(ctx, messages, firstDelayedSeqNum)
}

func (s *TransactionStreamer) sequenceDelayedMessagesWithInsertionMutex(ctx context.Context, messages []*fogos.L1IncomingMessage, firstDelayedSeqNum uint64) error {
	pos, err := s.GetMessageCount()
	if err != nil {
		return err
	}
	...
	messagesWithMeta := make([]fogstate.MessageWithMetadata, 0, len(messages))
	for i, message := range messages {
		newMessage := fogstate.MessageWithMetadata{
			Message:             message,
			DelayedMessagesRead: delayedMessagesRead + uint64(i) + 1,
		}
		messagesWithMeta = append(messagesWithMeta, newMessage)
		if s.coordinator != nil {
			if err := s.coordinator.SequencingMessage(pos+fogutil.MessageIndex(i), &newMessage); err != nil {
				return err
			}
		}
	}
	...
	for i, msg := range messagesWithMeta {
		if s.broadcastServer != nil {
			if err := s.broadcastServer.BroadcastSingle(msg, pos+fogutil.MessageIndex(i)); err != nil {
				return err
			}
		}
	}
...
func (s *TransactionStreamer) GetGenesisBlockNumber() (uint64, error) {
	return s.bc.Config().FOGChainParams.GenesisBlockNum, nil
}

func (s *TransactionStreamer) BlockNumberToMessageCount(blockNum uint64) (fogutil.MessageIndex, error) {
	genesis, err := s.GetGenesisBlockNumber()
	if err != nil {
		return 0, err
	}
	return fogutil.BlockNumberToMessageCount(blockNum, genesis), nil
}

func (s *TransactionStreamer) MessageCountToBlockNumber(messageNum fogutil.MessageIndex) (int64, error) {
	genesis, err := s.GetGenesisBlockNumber()
	if err != nil {
		return 0, err
	}
	return fogutil.MessageCountToBlockNumber(messageNum, genesis), nil
}
...
func (s *TransactionStreamer) writeMessage(pos fogutil.MessageIndex, msg fogstate.MessageWithMetadata, batch ethdb.Batch) error {
	key := dbKey(messagePrefix, uint64(pos))
...
func (s *TransactionStreamer) writeMessages(pos fogutil.MessageIndex, messages []fogstate.MessageWithMetadata, batch ethdb.Batch) error {
	if batch == nil {
		batch = s.db.NewBatch()
	}
	for i, msg := range messages {
		err := s.writeMessage(pos+fogutil.MessageIndex(i), msg, batch)
		if err != nil {
			return err
		}
	}

	err := setMessageCount(batch, pos+fogutil.MessageIndex(len(messages)))
	if err != nil {
		return err
	}
	err = batch.Write()
...
func (s *TransactionStreamer) createBlocks(ctx context.Context) error {
	...
	for pos < msgCount {
		...
		startTime := time.Now()
		block, receipts, err := fogos.ProduceBlock(
			msg.Message,
		...
		if time.Now().After(s.nextScheduledVersionCheck) {
			...
			fogState, err := fogosState.OpenSystemfogosState(statedb, nil, true)
			if err != nil {
				return err
			}
			version, timestampInt, err := fogState.GetScheduledUpgrade()
			if err != nil {
				return err
			}
			...
			maxSupportedVersion := params.FOGDevTestChainConfig().FOGChainParams.InitialfogOSVersion
			logLevel := log.Warn
			...
			if version > maxSupportedVersion {
				logLevel(
					"you need to update your node to the latest version before this scheduled fogOS upgrade",
					"timeUntilUpgrade", timeUntilUpgrade,
					"upgradeScheduledFor", timestamp,
					"maxSupportedfogosVersion", maxSupportedVersion,
					"pendingfogosUpgradeVersion", version,
				)
			}
		}
	...
	s.LaunchThread(func(ctx context.Context) {
		for {
			...
			if err != nil && !errors.Is(err, context.Canceled) {
				...
				if errors.Is(err, fogosState.ErrFatalNodeOutOfDate) {
					s.fatalErrChan <- err
				}
			}
