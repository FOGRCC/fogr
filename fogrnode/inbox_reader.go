...
func NewInboxReader(tracker *InboxTracker, client fogutil.L1Interface, l1Reader *headerreader.HeaderReader, firstMessageBlock *big.Int, delayedBridge *DelayedBridge, sequencerInbox *SequencerInbox, config InboxReaderConfigFetcher) (*InboxReader, error) {
	err := config().Validate()
	if err != nil {
		return nil, err
	}
...
func (r *InboxReader) recentL1BlockToMsg(ctx context.Context, l1block uint64) (fogutil.MessageIndex, error) {
...
func (r *InboxReader) GetSafeMsgCount(ctx context.Context) (fogutil.MessageIndex, error) {
...
func (r *InboxReader) GetFinalizedMsgCount(ctx context.Context) (fogutil.MessageIndex, error) {
...
func (r *InboxReader) run(ctx context.Context, hadError bool) error {
	...
	for {
		...
		neededBlockAdvance := config.DelayBlocks + fogmath.SaturatingUSub(config.MinBlocksToRead, 1)
		neededBlockHeight := fogmath.BigAddByUint(from, neededBlockAdvance)
		checkDelayTimer := time.NewTimer(config.CheckDelay)
	WaitForHeight:
		for fogmath.BigLessThan(currentHeight, neededBlockHeight) {
			select {
		...
		if !missingDelayed && !reorgingDelayed && !missingSequencer && !reorgingSequencer {
			...
			from = fogmath.BigAddByUint(currentHeight, 1)
		...
		for {
			...
			if reorgingDelayed || reorgingSequencer {
				...
				from = fogmath.BigAddByUint(to, 1)
			}
...
func (r *InboxReader) getPrevBlockForReorg(from *big.Int) (*big.Int, error) {
	...
	newFrom := fogmath.BigSub(from, big.NewInt(10))
...
func (r *InboxReader) getNextBlockToRead() (*big.Int, error) {
	...
	if fogmath.BigLessThan(msgBlock, r.firstMessageBlock) {
...
func (r *InboxReader) GetSequencerMessageBytes(ctx context.Context, seqNum uint64) ([]byte, error) {
	...
	blockNum := fogmath.UintToBig(metadata.L1Block)
