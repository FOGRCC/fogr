...
func PreCheckTx(chainConfig *params.ChainConfig, header *types.Header, statedb *state.StateDB, fogos *fogosState.fogosState, tx *types.Transaction, strictness uint) error {
	...
	if strictness < TxPreCheckerStrictnessLikelyCompatible {
		baseFee, err = fogos.L2PricingState().MinBaseFeeWei()
		if err != nil {
			return err
		}
	}
	if fogmath.BigLessThan(tx.GasFeeCap(), baseFee) {
	...
	cost := tx.Cost()
	if fogmath.BigLessThan(balance, cost) {
	...
	dataCost, _ := fogos.L1PricingState().GetPosterInfo(tx, l1pricing.BatchPosterAddress)
	dataGas := fogmath.BigDiv(dataCost, header.BaseFee)
...
func (c *TxPreChecker) PublishTransaction(ctx context.Context, tx *types.Transaction) error {
	...
	fogos, err := fogosState.OpenSystemfogosState(statedb, nil, true)
	if err != nil {
		return err
	}
	err = PreCheckTx(c.bc.Config(), block.Header(), statedb, fogos, tx, c.getStrictness())
	if err != nil {
		return err
	}
