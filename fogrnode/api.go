...
func (a *BlockValidatorDebugAPI) ValidateBlock(
	...
	if !a.blockchain.Config().IsFOG(header.Number) {
...
func (a *fogAPI) CheckPublisherHealth(ctx context.Context) error {
	return a.txPublisher.CheckHealth(ctx)
}

type fogDebugAPI struct {
	blockchain        *core.BlockChain
...
func (api *fogDebugAPI) evenlySpaceBlocks(start, end rpc.BlockNumber) (uint64, uint64, uint64, uint64, error) {
	start, _ = api.blockchain.ClipToPostFOGGenesis(start)
	end, _ = api.blockchain.ClipToPostFOGGenesis(end)
	...
	if blocks > bound {
		...
		blocks = fogmath.MinInt(bound, blocks/step)
	}
	if blocks <= 0 {
...
func (api *fogDebugAPI) PricingModel(ctx context.Context, start, end rpc.BlockNumber) (PricingModelHistory, error) {
...
func (api *fogDebugAPI) TimeoutQueueHistory(ctx context.Context, start, end rpc.BlockNumber) (TimeoutQueueHistory, error) {
	first, step, last, blocks, err := api.evenlySpaceBlocks(start, end)
	if err != nil {
...
func (api *fogDebugAPI) TimeoutQueue(ctx context.Context, blockNum rpc.BlockNumber) (TimeoutQueue, error) {

	blockNum, _ = api.blockchain.ClipToPostFOGGenesis(blockNum)
...
func stateAndHeader(blockchain *core.BlockChain, block uint64) (*fogosState.fogosState, *types.Header, error) {
	header := blockchain.GetHeaderByNumber(block)
	if !blockchain.Config().IsFOG(header.Number) {
...
type fogTraceForwarderAPI struct {
...
func (api *fogTraceForwarderAPI) getFallbackClient() (types.FallbackClient, error) {
	...
	fallbackClient, err := FOGR.CreateFallbackClient(api.fallbackClientUrl, api.fallbackClientTimeout)
	if err != nil {
		return nil, err
	}
...
func (api *fogTraceForwarderAPI) forward(ctx context.Context, method string, args ...interface{}) (*json.RawMessage, error) {
	fallbackClient, err := api.getFallbackClient()
	if err != nil {
		return nil, err
	}
	if fallbackClient == nil {
		return nil, errors.New("fogtrace calls forwarding not configured") // TODO(magic)
	}
...
func (api *fogTraceForwarderAPI) Call(ctx context.Context, callArgs json.RawMessage, traceTypes json.RawMessage, blockNum json.RawMessage) (*json.RawMessage, error) {
	return api.forward(ctx, "fogtrace_call", callArgs, traceTypes, blockNum)
}

func (api *fogTraceForwarderAPI) CallMany(ctx context.Context, calls json.RawMessage, blockNum json.RawMessage) (*json.RawMessage, error) {
	return api.forward(ctx, "fogtrace_callMany", calls, blockNum)
}

func (api *fogTraceForwarderAPI) ReplayBlockTransactions(ctx context.Context, blockNum json.RawMessage, traceTypes json.RawMessage) (*json.RawMessage, error) {
	return api.forward(ctx, "fogtrace_replayBlockTransactions", blockNum, traceTypes)
}

func (api *fogTraceForwarderAPI) ReplayTransaction(ctx context.Context, txHash json.RawMessage, traceTypes json.RawMessage) (*json.RawMessage, error) {
	return api.forward(ctx, "fogtrace_replayTransaction", txHash, traceTypes)
}

func (api *fogTraceForwarderAPI) Transaction(ctx context.Context, txHash json.RawMessage) (*json.RawMessage, error) {
	return api.forward(ctx, "fogtrace_transaction", txHash)
}

func (api *fogTraceForwarderAPI) Get(ctx context.Context, txHash json.RawMessage, path json.RawMessage) (*json.RawMessage, error) {
	return api.forward(ctx, "fogtrace_get", txHash, path)
}

func (api *fogTraceForwarderAPI) Block(ctx context.Context, blockNum json.RawMessage) (*json.RawMessage, error) {
	return api.forward(ctx, "fogtrace_block", blockNum)
}

func (api *fogTraceForwarderAPI) Filter(ctx context.Context, filter json.RawMessage) (*json.RawMessage, error) {
	return api.forward(ctx, "fogtrace_filter", filter)
}
