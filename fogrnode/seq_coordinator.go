...
var (
	isActiveSequencer = metrics.NewRegisteredGauge("fogr/sequencer/active", nil)
)
...
type SeqCoordinatorConfig struct {
	...
	MaxMsgPerPoll         fogutil.MessageIndex       `koanf:"msg-per-poll"`
...
func StandaloneSeqCoordinatorInvalidateMsgIndex(ctx context.Context, redisClient redis.UniversalClient, keyConfig string, msgIndex fogutil.MessageIndex) error {
...
func (c *SeqCoordinator) msgCountToSignedBytes(msgCount fogutil.MessageIndex) ([]byte, error) {
...
func (c *SeqCoordinator) signedBytesToMsgCount(ctx context.Context, data []byte) (fogutil.MessageIndex, error) {
	datalen := len(data)
	if datalen < 8 {
	...
	return fogutil.MessageIndex(binary.BigEndian.Uint64(msgCountBytes)), nil
}

// Acquires or refreshes the chosen one lockout and optionally writes a message into redis atomically.
func (c *SeqCoordinator) acquireLockoutAndWriteMessage(ctx context.Context, msgCountExpected, msgCountToWrite fogutil.MessageIndex, lastmsg *fogstate.MessageWithMetadata) error {
	...
	if lastmsg != nil {
		...
		msgSig, err := c.signer.SignMessage(fogmath.UintToBytes(uint64(msgCountToWrite-1)), msgBytes)
		if err != nil {
			return err
		}
...
func (c *SeqCoordinator) getRemoteMsgCountImpl(ctx context.Context, r redis.Cmdable) (fogutil.MessageIndex, error) {
...
func (c *SeqCoordinator) GetRemoteMsgCount() (fogutil.MessageIndex, error) {
...
func (c *SeqCoordinator) updateWithLockout(ctx context.Context, nextChosen string) time.Duration {
	...
	marginOfError := fogmath.MaxInt(c.config.LockoutDuration/5, c.config.UpdateInterval*5)
...
func (c *SeqCoordinator) update(ctx context.Context) time.Duration {
	...
	for msgToRead < readUntil {
		...
		msgReadErr = c.signer.VerifySignature(ctx, sigBytes, fogmath.UintToBytes(uint64(msgToRead)), rsBytes)
		if msgReadErr != nil {
		...
		if err != nil {
			...
			message = fogstate.MessageWithMetadata{
				Message:             fogstate.InvalidL1Message,
...
func (c *SeqCoordinator) SequencingMessage(pos fogutil.MessageIndex, msg *fogstate.MessageWithMetadata) error {
