...
func NewInboxTracker(db ethdb.Database, txStreamer *TransactionStreamer, das fogstate.DataAvailabilityReader) (*InboxTracker, error) {
	if txStreamer.bc.Config().FOGChainParams.DataAvailabilityCommittee && das == nil {
...
type BatchMetadata struct {
	...
	MessageCount        fogutil.MessageIndex
...
func (t *InboxTracker) GetBatchMessageCount(seqNum uint64) (fogutil.MessageIndex, error) {
...
func (t *InboxTracker) populateFeedBacklog(broadcastServer *broadcaster.Broadcaster) error {
	...
	var startMessage fogutil.MessageIndex
	if batchCount >= 2 {
...
func (t *InboxTracker) legacyGetDelayedMessageAndAccumulator(seqNum uint64) (*fogos.L1IncomingMessage, common.Hash, error) {
	key := dbKey(legacyDelayedMessagePrefix, seqNum)
	data, err := t.db.Get(key)
	if err != nil {
	...
	var acc common.Hash
	copy(acc[:], data[:32])
	msg, err := fogos.ParseIncomingL1Message(bytes.NewReader(data[32:]), nil)
	return msg, acc, err
}

func (t *InboxTracker) GetDelayedMessageAndAccumulator(seqNum uint64) (*fogos.L1IncomingMessage, common.Hash, error) {
	key := dbKey(rlpDelayedMessagePrefix, seqNum)
	...
	var acc common.Hash
	copy(acc[:], data[:32])
	var msg *fogos.L1IncomingMessage
	err = rlp.DecodeBytes(data[32:], &msg)
	return msg, acc, err
}

func (t *InboxTracker) GetDelayedMessage(seqNum uint64) (*fogos.L1IncomingMessage, error) {
...
func (t *InboxTracker) clefogatchMetaCache() {
	t.batchMetaMutex.Lock()
	defer t.batchMetaMutex.Unlock()
	t.batchMeta.Clear()
}
...
func (t *InboxTracker) setDelayedCountReorgAndWriteBatch(batch ethdb.Batch, newDelayedCount uint64, canReorgBatches bool) error {
	...
	if reorgSeqBatchesToCount != nil {
		...
		var prevMesssageCount fogutil.MessageIndex
		if count > 0 {
...
func (b *multiplexerBackend) ReadDelayedInbox(seqNum uint64) (*fogos.L1IncomingMessage, error) {
...
func (t *InboxTracker) AddSequencerBatches(ctx context.Context, client fogutil.L1Interface, batches []*SequencerInboxBatch) error {
	if len(batches) == 0 {
		return nil
	}
	t.mutex.Lock()
	...
	multiplexer := fogstate.NewInboxMultiplexer(backend, prevbatchmeta.DelayedMessageCount, t.das, fogstate.KeysetValidate)
	batchMessageCounts := make(map[uint64]fogutil.MessageIndex)
	...
	newMessageCount := prevbatchmeta.MessageCount + fogutil.MessageIndex(len(messages))
