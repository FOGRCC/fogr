...
import (
	...
	"github.com/FOGRCC/fogr/broadcastclient"
	"github.com/FOGRCC/fogr/broadcastclients"
	...
	"github.com/FOGRCC/fogr/solgen/go/challengegen"
	"github.com/FOGRCC/fogr/solgen/go/ospgen"
	"github.com/FOGRCC/fogr/solgen/go/rollupgen"
	"github.com/FOGRCC/fogr/staker"
	"github.com/FOGRCC/fogr/statetransfer"
	"github.com/FOGRCC/fogr/util/contracts"
	"github.com/FOGRCC/fogr/util/headerreader"
	"github.com/FOGRCC/fogr/util/signature"
	"github.com/FOGRCC/fogr/wsbroadcastserver"
)
...
func DeployOnL1(ctx context.Context, l1client fogutil.L1Interface, deployAuth *bind.TransactOpts, sequencer common.Address, authorizeValidators uint64, readerConfig headerreader.ConfigFetcher, config rollupgen.Config) (*RollupAddresses, error) {
...
func ConfigAddOptions(prefix string, f *flag.FlagSet, feedInputEnable bool, feedOutputEnable bool) {
	FOGR.ConfigAddOptions(prefix+".rpc", f)
...
type Node struct {
	...
	fogDB                   ethdb.Database
	Stack                   *node.Node
	Backend                 *FOGR.Backend
...
func checkfogDbSchemaVersion(fogDb ethdb.Database) error {
	var version uint64
	hasVersion, err := fogDb.Has(dbSchemaVersion)
	if err != nil {
		return err
	}
	if hasVersion {
		versionBytes, err := fogDb.Get(dbSchemaVersion)
		if err != nil {
			return err
		}
...
func createNodeImpl(
	...
	err := checkfogDbSchemaVersion(fogDb)
	if err != nil {
		return nil, err
	}
	...
	if config.Dangerous.ReorgToBlock >= 0 {
		...
		if blockNum < l2Config.FOGChainParams.GenesisBlockNum {
			return nil, fmt.Errorf("cannot reorg to block %v past fogr genesis of %v", blockNum, l2Config.FOGChainParams.GenesisBlockNum)
		}
	...
	if err != nil {
		if l2Config.FOGChainParams.GenesisBlockNum > 0 {
	...
	txStreamer, err := NewTransactionStreamer(fogDb, l2BlockChain, broadcastServer, fatalErrChan, transactionStreamerConfigFetcher)
	if err != nil {
		return nil, err
	}
	...
	dbs := []ethdb.Database{chainDb, fogDb}
	...
	fogInterface, err := NewfogInterface(txStreamer, txPublisher)
	if err != nil {
		return nil, err
	}
	...
	backend, filterSystem, err := FOGR.NewBackend(stack, &config.RPC, chainDb, fogInterface, syncMonitor, filterConfig)
	if err != nil {
		return nil, err
	}
	...
	} else if l2BlockChain.Config().FOGChainParams.DataAvailabilityCommittee {
		return nil, errors.New("a data availability service is required for this chain, but it was not configured")
	}

	inboxTracker, err := NewInboxTracker(fogDb, txStreamer, daReader)
	if err != nil {
		return nil, err
	}
	...
	if config.BlockValidator.URL != "" {
		statelessBlockValidator, err = staker.NewStatelessBlockValidator(
			...
			l2BlockChain,
			chainDb,
			rawdb.NewTable(fogDb, blockValidatorPrefix),
			daReader,
...
func CreateNode(
	...
	currentNode, err := createNodeImpl(ctx, stack, chainDb, fogDb, configFetcher, l2BlockChain, l1client, deployInfo, txOpts, dataSigner, fatalErrChan)
	if err != nil {
		return nil, err
	}
	var apis []rpc.API
	...
	apis = append(apis, rpc.API{
		Namespace: "fogr",
		Version:   "1.0",
		Service:   &fogAPI{currentNode.TxPublisher},
		Public:    false,
	})
	config := configFetcher.Get()
	apis = append(apis, rpc.API{
		...
		Service: &fogDebugAPI{
			blockchain:        l2BlockChain,
			blockRangeBound:   config.RPC.fogDebug.BlockRangeBound,
			timeoutQueueBound: config.RPC.fogDebug.TimeoutQueueBound,
		},
		Public: false,
	})
	apis = append(apis, rpc.API{
	...
	apis = append(apis, rpc.API{
		Namespace: "debug",
		Service:   eth.NewDebugAPI(eth.NewfogEthereum(l2BlockChain, chainDb)),
		Public:    false,
	})
...
func (n *Node) StopAndWait() {
	...
	n.fogInterface.BlockChain().Stop() // does nothing if not running
...
func WriteOrTestGenblock(chainDb ethdb.Database, initData statetransfer.InitDataReader, chainConfig *params.ChainConfig, accountsPerSync uint) error {
	...
	stateRoot, err := fogosState.InitializefogosInDatabase(chainDb, initData, chainConfig, timestamp, accountsPerSync)
	if err != nil {
		return err
	}

	genBlock := fogosState.MakeGenesisBlock(prevHash, blockNumber, timestamp, stateRoot, chainConfig)
