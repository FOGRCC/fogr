...
import (
	...
	"github.com/FOGRCC/fogr/fognode/dataposter"
...
type buildingBatch struct {
	...
	startMsgCount fogutil.MessageIndex
	msgCount      fogutil.MessageIndex
}
...
func (s *batchSegments) recompressAll() error {
	...
	if s.totalUncompressedSize > fogstate.MaxDecompressedLen {
		return fmt.Errorf("batch size %v exceeds maximum decompressed length %v", s.totalUncompressedSize, fogstate.MaxDecompressedLen)
	}
	if len(s.rawSegments) >= fogstate.MaxSegmentsPerSequencerMessage {
		return fmt.Errorf("number of raw segments %v excees maximum number %v", len(s.rawSegments), fogstate.MaxSegmentsPerSequencerMessage)
	}
	return nil
}
...
func (s *batchSegments) addL2Msg(l2msg []byte) (bool, error) {
	...
	segment[0] = fogstate.BatchSegmentKindL2Message
...
func (s *batchSegments) addDelayedMessage() (bool, error) {
	segment := []byte{fogstate.BatchSegmentKindDelayedMessages}
...
func (s *batchSegments) AddMessage(msg *fogstate.MessageWithMetadata) (bool, error) {
	if s.isDone {
		return false, errBatchAlreadyClosed
	}
	...
	success, err := s.maybeAddDiffSegment(&s.timestamp, msg.Message.Header.Timestamp, fogstate.BatchSegmentKindAdvanceTimestamp)
	if !success {
		return false, err
	}
	success, err = s.maybeAddDiffSegment(&s.blockNum, msg.Message.Header.BlockNumber, fogstate.BatchSegmentKindAdvanceL1BlockNumber)
	if !success {
		return false, err
	}
...
func (s *batchSegments) CloseAndGetBytes() ([]byte, error) {
	...
	fullMsg[0] = fogstate.BrotliMessageHeaderByte
	fullMsg = append(fullMsg, compressedBytes...)
	return fullMsg, nil
}

func (b *BatchPoster) encodeAddBatch(seqNum *big.Int, prevMsgNum fogutil.MessageIndex, newMsgNum fogutil.MessageIndex, message []byte, delayedMsg uint64) ([]byte, error) {
...
func (b *BatchPoster) maybePostSequencerBatch(ctx context.Context) (bool, error) {
	...
	for b.building.msgCount < msgCount {
		...
		if msg.Message.Header.Kind != fogos.L1MessageType_BatchPostingReport {
			haveUsefulMessage = true
		}
		b.building.msgCount++
	}
