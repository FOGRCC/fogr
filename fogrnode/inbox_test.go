...
import (
	...
	fogutil "github.com/FOGRCC/fogr/util"
...
func NewTransactionStreamerForTest(t *testing.T, ownerAddress common.Address) (*TransactionStreamer, ethdb.Database, *core.BlockChain) {
	...
	inbox, err := NewTransactionStreamer(fogDb, bc, nil, make(chan error, 1), transactionStreamerConfigFetcher)
	if err != nil {
		Fail(t, err)
	}
	...
	return inbox, fogDb, bc
}

type blockTestState struct {
	balances    map[common.Address]*big.Int
	accounts    []common.Address
	numMessages fogutil.MessageIndex
	blockNumber uint64
}
...
func TestTransactionStreamer(t *testing.T) {
	...
	blockStates = append(blockStates, blockTestState{
		balances: map[common.Address]*big.Int{
			ownerAddress: new(big.Int).Mul(maxExpectedGasCost, big.NewInt(int64(fogutil.NormalizeL2GasForL1GasInitial(1_000_000, params.GWei)))),
		},
	...
	for i := 1; i < 100; i++ {
		if i%10 == 0 {
			...
			for j := 0; j < numMessages; j++ {
				...
				var l2Message []byte
				l2Message = append(l2Message, fogos.L2MessageKind_ContractTx)
				...
				messages = append(messages, fogstate.MessageWithMetadata{
			...
			state.numMessages += fogutil.MessageIndex(len(messages))
