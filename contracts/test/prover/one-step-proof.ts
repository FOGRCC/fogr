...
async function sendTestMessages() {
  ...
  const msgRoot = '../fogitrator/prover/test-cases/rust/data/'
  const gasOpts = {
...
describe('OneStepProof', function () {
  const fogProofsRoot = './test/prover/proofs/'
  ...
  const proofs = []
  for (const file of fs.readdirSync(fogProofsRoot)) {
    if (!file.endsWith('.json')) continue
    proofs.push([fogProofsRoot + file, file])
  }
