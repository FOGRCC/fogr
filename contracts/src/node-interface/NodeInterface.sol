 ...
 *  This is a cute trick to allow an FOGR node to provide data without us having to implement additional RPCs
 */
interface NodeInterface {
     ...
     * @return l1BaseFeeEstimate fogOS's l1 estimate of the l1 base fee
     */
     ...
     * @return l2Sender sender if original message (i.e., caller of fogSys.sendTxToL1)
    ...
    // @notice Returns the first block produced using the FOGR codebase
    // @dev returns 0 for chains like Nova that don't contain classic blocks
    // @return number the block number
    function fogGenesisBlock() external pure returns (uint256 number);
}
