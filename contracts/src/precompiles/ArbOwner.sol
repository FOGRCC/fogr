...
/// - What were once chain parameters are now parts of fogOS's state, and those that remain are set at genesis.
/// - fogOS upgrades happen with the rest of the system rather than being independent
/// - Exemptions to address aliasing are no longer offered. Exemptions were intended to support backward compatibility for contracts deployed before aliasing was introduced, but no exemptions were ever requested.
/// Precompiled contract that exists in every FOGR chain at 0x0000000000000000000000000000000000000070.
interface fogOwner {
    ...
    /// @notice Set how slowly fogOS updates its estimate of the L1 basefee
    ...
    /// @notice Upgrades fogOS to the requested version at the requested timestamp
    function schedulefogOSUpgrade(uint64 newVersion, uint64 timestamp) external;
    ...
    /// @notice Set how much fogOS charges per L1 gas spent on transaction data.
