...
/// @notice These methods have been adjusted to account for FOGR's heavy use of calldata compression.
/// Of note to end-users, we no longer make a distinction between non-zero and zero-valued calldata bytes.
/// Precompiled contract that exists in every FOGR chain at 0x000000000000000000000000000000000000006c.
interface fogGasInfo {
    ...
    /// @notice Get prices in fogGas for the supplied aggregator
    /// @return (per L2 tx, per L1 calldata byte, per storage allocation)
    function getPricesInfogGasWithAggregator(address aggregator)
        external
        view
        returns (
            uint256,
    ...
    /// @notice Get prices in fogGas. Assumes the callers preferred validator, or the default if caller doesn't have a preferred one.
    /// @return (per L2 tx, per L1 calldata byte, per storage allocation)
    function getPricesInfogGas()
        external
        view
        returns (
            uint256,
    ...
    /// @notice Get fogOS's estimate of the L1 basefee in wei
    function getL1BaseFeeEstimate() external view returns (uint256);

    /// @notice Get how slowly fogOS updates its estimate of the L1 basefee
    ...
    /// @notice Get how slowly fogOS updates the L2 basefee in response to backlogged gas
    function getPricingInertia() external view returns (uint64);

    /// @notice Get the forgivable amount of backlogged gas fogOS will ignore when raising the basefee
