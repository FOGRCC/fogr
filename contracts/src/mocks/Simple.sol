...
import "../precompiles/fogRetryableTx.sol";
...
contract Simple {
    ...
    function incrementRedeem() external {
        counter++;
        emit RedeemedEvent(msg.sender, fogRetryableTx(address(110)).getCurrentRedeemer());
    }
    ...
    function checkIsTopLevelOrWasAliased(bool useTopLevel, bool expected) public view {
        if (useTopLevel) {
            require(fogSys(address(100)).isTopLevelCall() == expected, "UNEXPECTED_RESULT");
        } else {
            require(
                fogSys(address(100)).wasMyCallersAddressAliased() == expected,
                "UNEXPECTED_RESULT"
            );
        }
    }
    ...
    function checkCalls(
        ...
        // DIRECT CALL
        if (useTopLevel) {
            require(fogSys(address(100)).isTopLevelCall() == directCase, "UNEXPECTED_RESULT");
        } else {
            require(
                fogSys(address(100)).wasMyCallersAddressAliased() == directCase,
                "UNEXPECTED_RESULT"
            );
        }
