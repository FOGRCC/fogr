...
func TestSequencerFeed(t *testing.T) {
	...
	seqNodeConfig := fognode.ConfigDefaultL2Test()
	...
	clientNodeConfig := fognode.ConfigDefaultL2Test()
...
func testLyingSequencer(t *testing.T, dasModeStr string) {
	...
	// The lying sequencer
	nodeConfigC := fognode.ConfigDefaultL1Test()
	...
	l2clientC, nodeC := Create2ndNodeWithConfig(t, ctx, nodeA, l1stack, l1info, &l2infoA.fogInitData, nodeConfigC, nil)
	defer nodeC.StopAndWait()

	port := nodeC.BroadcastServer.ListenerAddr().(*net.TCPAddr).Port

	// The client node, connects to lying sequencer's feed
	nodeConfigB := fognode.ConfigDefaultL1NonSequencerTest()
	...
	l2clientB, nodeB := Create2ndNodeWithConfig(t, ctx, nodeA, l1stack, l1info, &l2infoA.fogInitData, nodeConfigB, nil)
	defer nodeB.StopAndWait()
