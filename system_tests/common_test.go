...
import (
	...
	"github.com/FOGRCC/fogr/validator/valnode"

	"github.com/FOGRCC/fogr/blsSignatures"
	...
	"github.com/FOGRCC/fogr/util/headerreader"
	"github.com/FOGRCC/fogr/util/signature"
	...
	_ "github.com/FOGRCC/fogr/nodeInterface"
	...
	"github.com/FOGRCC/fogr/statetransfer"
	"github.com/FOGRCC/fogr/util/testhelpers"
)

type info = *BlockchainTestInfo
type client = fogutil.L1Interface
...
func SendSignedTxViaL1(
	...
	l1client fogutil.L1Interface,
	l2client fogutil.L1Interface,
	...
	Require(t, err)
	txwrapped := append([]byte{fogos.L2MessageKind_SignedTx}, txbytes...)
...
func SendUnsignedTxViaL1(
	...
	unsignedTx := types.NewTx(&types.FOGUnsignedTx{
	...
	l1tx, err := delayedInboxContract.SendUnsignedTransaction(
		&usertxopts,
		fogmath.UintToBig(unsignedTx.Gas()),
		unsignedTx.GasFeeCap(),
		fogmath.UintToBig(unsignedTx.Nonce()),
		*unsignedTx.To(),
		unsignedTx.Value(),
		unsignedTx.Data(),
	)
...
func configByValidationNode(t *testing.T, clientConfig *fognode.Config, valStack *node.Node) {
	clientConfig.BlockValidator.URL = valStack.WSEndpoint()
	clientConfig.BlockValidator.JWTSecret = ""
}

func AddDefaultValNode(t *testing.T, ctx context.Context, nodeConfig *fognode.Config, useJit bool) {
...
func createTestL1BlockChainWithConfig(t *testing.T, l1info info, stackConfig *node.Config) (info, *ethclient.Client, *eth.Ethereum, *node.Node) {
	...
	chainConfig.FOGChainParams = params.FOGChainParams{}
...
func DeployOnTestL1(
	...
	Require(t, err)
	config := fognode.GenerateRollupConfig(false, locator.LatestWasmModuleRoot(), l1info.GetAddress("RollupOwner"), chainId, common.Address{})
	addresses, err := fognode.DeployOnL1(
		ctx,
		l1client,
		&l1TransactionOpts,
...
func createL2BlockChainWithStackConfig(
	...
	if l2info == nil {
		l2info = NewfogTestInfo(t, chainConfig.ChainID)
	}
	var stack *node.Node
	var err error
	if stackConfig == nil {
	...
	Require(t, err)
	fogDb, err := stack.OpenDatabase("fogdb", 0, 0, "", false)
	Require(t, err)

	initReader := statetransfer.NewMemoryInitDataReader(&l2info.fogInitData)
	blockchain, err := fognode.WriteOrTestBlockChain(chainDb, nil, initReader, chainConfig, fognode.ConfigDefaultL2Test(), 0)
	Require(t, err)

	return l2info, stack, chainDb, fogDb, blockchain
}
...
// Create and deploy L1 and fognode for L2
func createTestNodeOnL1(
	t *testing.T,
	ctx context.Context,
	isSequencer bool,
) (
	l2info info, node *fognode.Node, l2client *ethclient.Client, l1info info,
...
func createTestNodeOnL1WithConfig(
	...
	ctx context.Context,
	isSequencer bool,
	nodeConfig *fognode.Config,
	chainConfig *params.ChainConfig,
	stackConfig *node.Config,
) (
	l2info info, currentNode *fognode.Node, l2client *ethclient.Client, l1info info,
	l1backend *eth.Ethereum, l1client *ethclient.Client, l1stack *node.Node,
) {
...
func createTestNodeOnL1WithConfigImpl(
	...
	l2info info, currentNode *fognode.Node, l2client *ethclient.Client, l2stack *node.Node,
	l1info info, l1backend *eth.Ethereum, l1client *ethclient.Client, l1stack *node.Node,
) {
	if nodeConfig == nil {
		nodeConfig = fognode.ConfigDefaultL1Test()
	}
	if chainConfig == nil {
		chainConfig = params.FOGDevTestChainConfig()
	}
	...
	var l2fogDb ethdb.Database
	var l2blockchain *core.BlockChain
	l2info, l2stack, l2chainDb, l2fogDb, l2blockchain = createL2BlockChainWithStackConfig(t, nil, "", chainConfig, stackConfig)
	...
	var err error
	currentNode, err = fognode.CreateNode(
		ctx, l2stack, l2chainDb, l2fogDb, nodeConfig, l2blockchain, l1client,
...
func CreateTestL2(t *testing.T, ctx context.Context) (*BlockchainTestInfo, *fognode.Node, *ethclient.Client) {
	return CreateTestL2WithConfig(t, ctx, nil, fognode.ConfigDefaultL2Test(), true)
}

func CreateTestL2WithConfig(
	t *testing.T, ctx context.Context, l2Info *BlockchainTestInfo, nodeConfig *fognode.Config, takeOwnership bool,
) (*BlockchainTestInfo, *fognode.Node, *ethclient.Client) {
	feedErrChan := make(chan error, 10)

	AddDefaultValNode(t, ctx, nodeConfig, true)

	l2info, stack, chainDb, fogDb, blockchain := createL2BlockChain(t, l2Info, "", params.FOGDevTestChainConfig())
	currentNode, err := fognode.CreateNode(ctx, stack, chainDb, fogDb, nodeConfig, blockchain, nil, nil, nil, nil, feedErrChan)
	Require(t, err)
	...
	if takeOwnership {
		...
		Require(t, err, "failed to deploy fogDebug")

		tx, err := fogdebug.BecomeChainOwner(&debugAuth)
		Require(t, err, "failed to deploy fogDebug")
...
func StartWatchChanErr(t *testing.T, ctx context.Context, feedErrChan chan error, node *fognode.Node) {
	go func() {
		select {
		case <-ctx.Done():
			return
...
func Create2ndNode(
	...
	l1stack *node.Node,
	l1info *BlockchainTestInfo,
	l2InitData *statetransfer.fogosInitializationInfo,
	dasConfig *das.DataAvailabilityConfig,
) (*ethclient.Client, *fognode.Node) {
	nodeConf := fognode.ConfigDefaultL1NonSequencerTest()
	if dasConfig == nil {
...
func Create2ndNodeWithConfig(
	...
	Require(t, err)
	l2fogDb, err := l2stack.OpenDatabase("fogdb", 0, 0, "", false)
	Require(t, err)
	...
	l2blockchain, err := fognode.WriteOrTestBlockChain(l2chainDb, nil, initReader, first.fogInterface.BlockChain().Config(), fognode.ConfigDefaultL2Test(), 0)
	Require(t, err)

	AddDefaultValNode(t, ctx, nodeConfig, true)

	currentNode, err := fognode.CreateNode(ctx, l2stack, l2chainDb, l2fogDb, nodeConfig, l2blockchain, l1client, first.DeployInfo, &txOpts, dataSigner, feedErrChan)
	Require(t, err)
...
func authorizeDASKeyset(
	...
	keyset := &fogstate.DataAvailabilityKeyset{
		AssumedHonest: 1,
...
func setupConfigWithDAS(
...
) (*params.ChainConfig, *fognode.Config, *das.LifecycleManager, string, *blsSignatures.PublicKey) {
	l1NodeConfigA := fognode.ConfigDefaultL1Test()
	chainConfig := params.FOGDevTestChainConfig()
	var dbPath string
	var err error
	...
	case "db":
		...
		chainConfig = params.FOGDevTestDASChainConfig()
	case "files":
