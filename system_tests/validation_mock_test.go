...
func createMockValidationNode(t *testing.T, ctx context.Context, config *server_fog.fogitratorSpawnerConfig) *node.Node {
	...
	if config == nil {
		config = &server_fog.DefaultfogitratorSpawnerConfig
	}
	configFetcher := func() *server_fog.fogitratorSpawnerConfig { return config }
	spawner := &mockSpawner{}
...
func TestExecutionKeepAlive(t *testing.T) {
	...
	defer cancel()
	validationDefault := createMockValidationNode(t, ctx, nil)
	shortTimeoutConfig := server_fog.DefaultfogitratorSpawnerConfig
