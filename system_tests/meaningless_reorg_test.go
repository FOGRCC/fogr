...
func TestMeaninglessBatchReorg(t *testing.T) {
	...
	l2Info, fogNode, l2Client, l1Info, l1Backend, l1Client, l1stack := createTestNodeOnL1WithConfig(t, ctx, true, conf, nil, nil)
	...
	for i := 0; ; i++ {
		...
		batchCount, err := fogNode.InboxTracker.GetBatchCount()
		Require(t, err)
		if batchCount == 2 {
			break
	...
	metadata, err := fogNode.InboxTracker.GetBatchMetadata(1)
	Require(t, err)
	...
	_, err = fogNode.InboxReader.GetSequencerMessageBytes(ctx, 1)
	Require(t, err)
