...
func FuzzPrecompiles(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, precompileSelector byte, methodSelector byte, input []byte) {
		...
		_, err = fogosState.InitializefogosState(sdb, burner, params.FOGDevTestChainConfig())
		if err != nil {
			panic(err)
		}
		...
		evm := vm.NewEVM(blockContext, txContext, sdb, params.FOGDevTestChainConfig(), vm.Config{})
