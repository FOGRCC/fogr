...
func TestRedisSeqCoordinatorPriorities(t *testing.T) {
	...
	trySequencing := func(nodeNum int) bool {
		...
		Require(t, err)
		emptyMessage := fogstate.MessageWithMetadata{
		...
		if errors.Is(err, fognode.ErrRetrySequencer) {
			return false
		}
		Require(t, err)
		Require(t, node.TxStreamer.AddMessages(curMsgs, false, []fogstate.MessageWithMetadata{emptyMessage}))
		return true
	}
	...
	waitForMsgEverywhere := func(msgNum fogutil.MessageIndex) {
	...
	nodeForwardTarget := func(nodeNum int) int {
		fwTarget := nodes[nodeNum].TxPublisher.(*fognode.TxPreChecker).TransactionPublisher.(*fognode.Sequencer).ForwardTarget()
		if fwTarget == "" {
			return -1
		}
	...
	messagesPerRound := fogutil.MessageIndex(10)
	currentSequencer := 0
	sequencedMesssages := fogutil.MessageIndex(1) // we start with 1 so messageCountKey will be written
	...
	for {
		...
		for i := fogutil.MessageIndex(0); i < messagesPerRound; i++ {
...
func testCoordinatorMessageSync(t *testing.T, successCase bool) {
	...
	l2Info, nodeA, clientA, l1info, _, _, l1stack := createTestNodeOnL1WithConfig(t, ctx, true, nodeConfig, params.FOGDevTestChainConfig(), nil)
	...
	clientB, nodeB := Create2ndNodeWithConfig(t, ctx, nodeA, l1stack, l1info, &l2Info.fogInitData, nodeConfig, nil)
