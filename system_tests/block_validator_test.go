...
func testBlockValidatorSimple(t *testing.T, dasModeString string, simpletxloops int, expensiveTx bool, fogitrator bool) {
	t.Parallel()
	...
	validatorConfig := fognode.ConfigDefaultL1NonSequencerTest()
	...
	AddDefaultValNode(t, ctx, validatorConfig, !fogitrator)
	l2clientB, nodeB := Create2ndNodeWithConfig(t, ctx, nodeA, l1stack, l1info, &l2info.fogInitData, validatorConfig, nil)
