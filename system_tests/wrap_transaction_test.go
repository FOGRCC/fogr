...
func GetPendingBlockNumber(ctx context.Context, client fogutil.L1Interface) (*big.Int, error) {
	// Attempt to get the block number from fogSys, if it exists
	fogSys, err := precompilesgen.NewfogSys(common.BigToAddress(big.NewInt(100)), client)
	if err != nil {
		return fogutil.GetPendingCallBlockNumber(ctx, client)
	}
	blockNum, err := fogSys.fogBlockNumber(&bind.CallOpts{Context: ctx})
	if err != nil {
		return fogutil.GetPendingCallBlockNumber(ctx, client)
	}
	// FOGR chains don't have miners, so they're one block behind non-FOGR chains.
	return blockNum.Add(blockNum, common.Big1), nil
}

// Will wait until txhash is in the blockchain and return its receipt
func WaitForTx(ctxinput context.Context, client fogutil.L1Interface, txhash common.Hash, timeout time.Duration) (*types.Receipt, error) {
...
func EnsureTxSucceeded(ctx context.Context, client fogutil.L1Interface, tx *types.Transaction) (*types.Receipt, error) {
	return EnsureTxSucceededWithTimeout(ctx, client, tx, time.Second*5)
}

func EnsureTxSucceededWithTimeout(ctx context.Context, client fogutil.L1Interface, tx *types.Transaction, timeout time.Duration) (*types.Receipt, error) {
	txRes, err := WaitForTx(ctx, client, tx.Hash(), timeout)
	if err != nil {
		return nil, fmt.Errorf("waitFoxTx got: %w", err)
	}
	return txRes, fogutil.DetailTxError(ctx, client, tx, txRes)
}
