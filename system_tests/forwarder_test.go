...
func TestStaticForwarder(t *testing.T) {
	...
	nodeConfigA := fognode.ConfigDefaultL1Test()
	...
	nodeConfigB := fognode.ConfigDefaultL1Test()
	...
	clientB, nodeB := Create2ndNodeWithConfig(t, ctx, nodeA, l1stack, l1info, &l2info.fogInitData, nodeConfigB, nil)
...
) (l2info info, currentNode *fognode.Node, l2client *ethclient.Client,
...
func TestRedisForwarder(t *testing.T) {
	...
	forwardingClient, forwardingNode := createForwardingNode(t, ctx, fallbackNode, l1stack, l1info, &l2info.fogInitData, "", redisUrl, fallbackIpcPath)
	defer forwardingNode.StopAndWait()

	var sequencers []*fognode.Node
	var seqClients []*ethclient.Client
	for _, path := range nodePaths {
		client, node := createSequencer(t, ctx, fallbackNode, l1stack, l1info, &l2info.fogInitData, path, redisUrl)
	...
	for i := range clients {
		...
		for j := 0; j < 20; j++ {
			...
			time.Sleep(fognode.DefaultTestForwarderConfig.UpdateInterval / 2)
		}
		testhelpers.RequireImpl(t, err)
