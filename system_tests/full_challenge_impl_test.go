...
func writeTxToBatch(writer io.Writer, tx *types.Transaction) error {
	...
	var segment []byte
	segment = append(segment, fogstate.BatchSegmentKindL2Message)
	segment = append(segment, fogos.L2MessageKind_SignedTx)
	segment = append(segment, txData...)
	err = rlp.Encode(writer, segment)
	return err
}

func makeBatch(t *testing.T, l2Node *fognode.Node, l2Info *BlockchainTestInfo, backend *ethclient.Client, sequencer *bind.TransactOpts, seqInbox *mocksgen.SequencerInboxStub, seqInboxAddr common.Address, isChallenger bool) {
	...
	compressed, err := fogcompress.CompressWell(batchBuffer.Bytes())
	Require(t, err)
	...
	nodeSeqInbox, err := fognode.NewSequencerInbox(backend, seqInboxAddr, 0)
	Require(t, err)
...
func confirmLatestBlock(ctx context.Context, t *testing.T, l1Info *BlockchainTestInfo, backend fogutil.L1Interface) {
	for i := 0; i < 12; i++ {
...
func setupSequencerInboxStub(ctx context.Context, t *testing.T, l1Info *BlockchainTestInfo, l1Client fogutil.L1Interface, chainConfig *params.ChainConfig) (common.Address, *mocksgen.SequencerInboxStub, common.Address) {
...
func RunChallengeTest(t *testing.T, asserterIsCorrect bool) {
	...
	asserterL2Info, asserterL2Stack, asserterL2ChainDb, asserterL2fogDb, asserterL2Blockchain := createL2BlockChain(t, nil, "", chainConfig)
	asserterRollupAddresses.Bridge = asserterBridgeAddr
	asserterRollupAddresses.SequencerInbox = asserterSeqInboxAddr
	asserterL2, err := fognode.CreateNode(ctx, asserterL2Stack, asserterL2ChainDb, asserterL2fogDb, conf, asserterL2Blockchain, l1Backend, asserterRollupAddresses, nil, nil, fatalErrChan)
	Require(t, err)
	err = asserterL2.Start(ctx)
	Require(t, err)

	challengerL2Info, challengerL2Stack, challengerL2ChainDb, challengerL2fogDb, challengerL2Blockchain := createL2BlockChain(t, nil, "", chainConfig)
	...
	challengerL2, err := fognode.CreateNode(ctx, challengerL2Stack, challengerL2ChainDb, challengerL2fogDb, conf, challengerL2Blockchain, l1Backend, &challengerRollupAddresses, nil, nil, fatalErrChan)
	Require(t, err)
	...
	asserterGenesis := asserterL2.fogInterface.BlockChain().Genesis()
	challengerGenesis := challengerL2.fogInterface.BlockChain().Genesis()
	if asserterGenesis.Hash() != challengerGenesis.Hash() {
		Fail(t, "asserter and challenger have different genesis hashes")
	}
	asserterLatestBlock := asserterL2.fogInterface.BlockChain().CurrentBlock()
	challengerLatestBlock := challengerL2.fogInterface.BlockChain().CurrentBlock()
	...
	asserterValidator, err := staker.NewStatelessBlockValidator(asserterL2.InboxReader, asserterL2.InboxTracker, asserterL2.TxStreamer, asserterL2Blockchain, asserterL2ChainDb, asserterL2fogDb, nil, &conf.BlockValidator)
	if err != nil {
		Fail(t, err)
	}
	...
	challengerValidator, err := staker.NewStatelessBlockValidator(challengerL2.InboxReader, challengerL2.InboxTracker, challengerL2.TxStreamer, challengerL2Blockchain, challengerL2ChainDb, challengerL2fogDb, nil, &conf.BlockValidator)
	if err != nil {
		Fail(t, err)
	}
