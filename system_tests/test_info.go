...
type BlockchainTestInfo struct {
	...
	fogInitData statetransfer.fogosInitializationInfo
	GasPrice    *big.Int
...
func NewfogTestInfo(t *testing.T, chainId *big.Int) *BlockchainTestInfo {
	var transferGas = util.NormalizeL2GasForL1GasInitial(800_000, params.GWei) // include room for aggregator L1 costs
	foginfo := NewBlockChainTestInfo(
		t,
		types.NewFOGSigner(types.NewLondonSigner(chainId)), big.NewInt(l2pricing.InitialBaseFeeWei*2),
		transferGas,
	)
	foginfo.GenerateGenesisAccount("Owner", new(big.Int).Sub(new(big.Int).Lsh(big.NewInt(1), 256), big.NewInt(9)))
	foginfo.GenerateGenesisAccount("Faucet", new(big.Int).Sub(new(big.Int).Lsh(big.NewInt(1), 256), big.NewInt(9)))
	return foginfo
}
...
func (b *BlockchainTestInfo) GenerateGenesisAccount(name string, balance *big.Int) {
	...
	b.fogInitData.Accounts = append(b.fogInitData.Accounts, statetransfer.AccountInitializationInfo{
...
func (b *BlockchainTestInfo) GetGenesisAlloc() core.GenesisAlloc {
	...
	for _, info := range b.fogInitData.Accounts {
