...
func TestOutboxProofs(t *testing.T) {
	...
	fogSysAbi, err := precompilesgen.fogSysMetaData.GetAbi()
	Require(t, err, "failed to get abi")
	withdrawTopic := fogSysAbi.Events["L2ToL1Tx"].ID
	merkleTopic := fogSysAbi.Events["SendMerkleUpdate"].ID
	...
	for i := int64(0); i < txnCount; i++ {
		...
		if err == nil {
			merkleState, err := fogSys.SendMerkleTreeState(&bind.CallOpts{})
	...
	for _, tx := range txns {
		...
		for _, log := range receipt.Logs {
			...
			if log.Topics[0] == withdrawTopic {
				parsedLog, err := fogSys.ParseL2ToL1Tx(*log)
	...
	for _, root := range roots {
		...
		balanced := treeSize == fogmath.NextPowerOf2(treeSize)/2
		treeLevels := int(fogmath.Log2ceil(treeSize)) // the # of levels in the tree
