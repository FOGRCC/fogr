...
func testSequencerInboxReaderImpl(t *testing.T, validator bool) {
	...
	l2Info, fogNode, _, l1Info, l1backend, l1Client, l1stack := createTestNodeOnL1WithConfig(t, ctx, false, conf, nil, nil)
	l2Backend := fogNode.Backend
	defer requireClose(t, l1stack)
	defer fogNode.StopAndWait()
	...
	for i := 1; i < seqInboxTestIters; i++ {
		...
		if validator && i%15 == 0 {
			for i := 0; ; i++ {
				lastValidated := fogNode.BlockValidator.LastBlockValidated()
