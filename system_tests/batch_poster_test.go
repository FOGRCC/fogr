...
func TestBatchPosterParallel(t *testing.T) {
	...
	l2clientB, nodeB := Create2ndNode(t, ctx, nodeA, l1stack, l1info, &l2info.fogInitData, nil)
	...
	for i := 0; i < parallelBatchPosters; i++ {
		batchPoster, err := fognode.NewBatchPoster(nodeA.L1Reader, nodeA.InboxTracker, nodeA.TxStreamer, nodeA.SyncMonitor, func() *fognode.BatchPosterConfig { return &conf.BatchPoster }, nodeA.DeployInfo, &seqTxOpts, nil)
		Require(t, err)
	...
	if false {
		...
		Require(t, err)
		seqInbox, err := fognode.NewSequencerInbox(l1client, nodeA.DeployInfo.SequencerInbox, 0)
		Require(t, err)
