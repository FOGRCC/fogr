...
func TestDASRekey(t *testing.T) {
	...
	l1NodeConfigB := fognode.ConfigDefaultL1NonSequencerTest()
...

	{
		...
		// Setup L2 chain
		_, l2stackA, l2chainDb, l2fogDb, l2blockchain := createL2BlockChain(t, l2info, nodeDir, chainConfig)
		...
		nodeA, err := fognode.CreateNode(ctx, l2stackA, l2chainDb, l2fogDb, l1NodeConfigA, l2blockchain, l1client, addresses, sequencerTxOptsPtr, nil, feedErrChan)
		Require(t, err)
		...
		l2clientB, nodeB := Create2ndNodeWithConfig(t, ctx, nodeA, l1stack, l1info, &l2info.fogInitData, l1NodeConfigB, nil)
	...
	l2fogDb, err := l2stackA.OpenDatabase("fogdb", 0, 0, "", false)
	Require(t, err)

	l2blockchain, err := fognode.GetBlockChain(l2chainDb, nil, chainConfig, fognode.ConfigDefaultL2Test())
	Require(t, err)
...
func TestDASComplexConfigAndRestMirror(t *testing.T) {
	...
	// Setup L2 chain
	l2info, l2stackA, l2chainDb, l2fogDb, l2blockchain := createL2BlockChain(t, nil, "", chainConfig)
	l2info.GenerateAccount("User2")

	sequencerTxOpts := l1info.GetDefaultTransactOpts("Sequencer", ctx)
	sequencerTxOptsPtr := &sequencerTxOpts
	nodeA, err := fognode.CreateNode(ctx, l2stackA, l2chainDb, l2fogDb, l1NodeConfigA, l2blockchain, l1client, addresses, sequencerTxOptsPtr, dataSigner, feedErrChan)
	Require(t, err)
	Require(t, nodeA.Start(ctx))
	l2clientA := ClientForStack(t, l2stackA)
