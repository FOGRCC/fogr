...
func TestInitContract(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		l2info.fogInitData.Accounts = append(l2info.fogInitData.Accounts, accountInfo)
		expectedSums[accountAddress] = sum
	}
	_, node, client := CreateTestL2WithConfig(t, ctx, l2info, fognode.ConfigDefaultL2Test(), true)
