...
var (
	...
	HTTPHeaderFeedServerVersion       = textproto.CanonicalMIMEHeaderKey("FOGR-Feed-Server-Version")
	HTTPHeaderFeedClientVersion       = textproto.CanonicalMIMEHeaderKey("FOGR-Feed-Client-Version")
	HTTPHeaderRequestedSequenceNumber = textproto.CanonicalMIMEHeaderKey("FOGR-Requested-Sequence-Number")
	HTTPHeaderChainId                 = textproto.CanonicalMIMEHeaderKey("FOGR-Chain-Id")
)
...
func (s *WSBroadcastServer) StartWithHeader(ctx context.Context, header ws.HandshakeHeader) error {
	...
	handle := func(conn net.Conn) {
		...
		var requestedSeqNum fogutil.MessageIndex
		upgrader := ws.Upgrader{
			...
			OnHeader: func(key []byte, value []byte) error {
				...
				if headerName == HTTPHeaderFeedClientVersion {
					...
					requestedSeqNum = fogutil.MessageIndex(num)
	...
	log.Info("FOGR websocket broadcast server is listening", "address", ln.Addr().String())
