...
func NewBlockChallengeBackend(
	...
	startBlockNum := fogutil.MessageCountToBlockNumber(0, genesisBlockNumber)
	...
	var startMsgCount fogutil.MessageIndex
	if startGs.Batch > 0 {
		var err error
	...
	startMsgCount += fogutil.MessageIndex(startGs.PosInBatch)
	expectedMsgCount := fogutil.SignedBlockNumberToMessageCount(startBlockNum, genesisBlockNumber)
	...
	var endMsgCount fogutil.MessageIndex
	if endGs.Batch > 0 {
		var err error
	...
	endMsgCount += fogutil.MessageIndex(endGs.PosInBatch)
...
func (b *BlockChallengeBackend) findBatchFromMessageIndex(msgCount fogutil.MessageIndex) (uint64, error) {
	if msgCount == 0 {
		return 0, nil
	}
	low := b.startGs.Batch
...
func (b *BlockChallengeBackend) FindGlobalStateFromHeader(header *types.Header) (validator.GoGlobalState, error) {
	...
	msgCount := fogutil.BlockNumberToMessageCount(header.Number.Uint64(), b.genesisBlockNumber)
	batch, err := b.findBatchFromMessageIndex(msgCount)
	if err != nil {
		return validator.GoGlobalState{}, err
	}
	var batchMsgCount fogutil.MessageIndex
	if batch > 0 {
