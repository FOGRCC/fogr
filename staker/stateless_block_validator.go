...
type StatelessBlockValidator struct {
	...
	db                ethdb.Database
	daService         fogstate.DataAvailabilityReader
	genesisBlockNum   uint64
	recordingDatabase *FOGR.RecordingDatabase
...
type InboxTrackerInterface interface {
	...
	GetBatchMessageCount(seqNum uint64) (fogutil.MessageIndex, error)
...
type TransactionStreamerInterface interface {
	BlockValidatorRegistrer
	GetMessage(seqNum fogutil.MessageIndex) (*fogstate.MessageWithMetadata, error)
...
func GlobalStatePositionsFor(
	...
	var firstInBatch fogutil.MessageIndex
	if batch > 0 {
...
func FindBatchContainingMessageIndex(
	tracker InboxTrackerInterface, pos fogutil.MessageIndex, high uint64,
) (uint64, error) {
	var low uint64
...
type validationEntry struct {
	...
	HasDelayedMsg   bool
	DelayedMsgNr    uint64
	msg             *fogstate.MessageWithMetadata
...
func NewStatelessBlockValidator(
	...
	das fogstate.DataAvailabilityReader,
	config *BlockValidatorConfig,
	...
	validator := &StatelessBlockValidator{
		...
		recordingDatabase:  FOGR.NewRecordingDatabase(blockchainDb, blockchain),
	}
...
func (v *StatelessBlockValidator) RecordBlockCreation(
	...
	if prevHeader != nil {
		initialfogosState, err := fogosState.OpenSystemfogosState(recordingdb, nil, true)
		if err != nil {
			return common.Hash{}, nil, nil, fmt.Errorf("error opening initial fogOS state: %w", err)
		}
		chainId, err := initialfogosState.ChainId()
		if err != nil {
			return common.Hash{}, nil, nil, fmt.Errorf("error getting chain ID from initial fogOS state: %w", err)
		}
		if chainId.Cmp(chainConfig.ChainID) != 0 {
			return common.Hash{}, nil, nil, fmt.Errorf("unexpected chain ID %v in fogOS state, expected %v", chainId, chainConfig.ChainID)
		}
		genesisNum, err := initialfogosState.GenesisBlockNum()
		if err != nil {
			return common.Hash{}, nil, nil, fmt.Errorf("error getting genesis block number from initial fogOS state: %w", err)
		}
		expectedNum := chainConfig.FOGChainParams.GenesisBlockNum
		if genesisNum != expectedNum {
			return common.Hash{}, nil, nil, fmt.Errorf("unexpected genesis block number %v in fogOS state, expected %v", genesisNum, expectedNum)
		}
	}
...
func (v *StatelessBlockValidator) ValidationEntryAddSeqMessage(ctx context.Context, e *validationEntry,
	...
	for _, batch := range e.BatchInfo {
		...
		if !fogstate.IsDASMessageHeaderByte(batch.Data[40]) {
			continue
		}
		if v.daService == nil {
			log.Error("No DAS configured, but sequencer message found with DAS header")
			if v.blockchain.Config().FOGChainParams.DataAvailabilityCommittee {
				return errors.New("processing data availability chain without DAS configured")
			}
		} else {
			_, err := fogstate.RecoverPayloadFromDasBatch(
				ctx, batch.Number, batch.Data, v.daService, e.Preimages, fogstate.KeysetValidate,
			)
			if err != nil {
				return err
			}
...
func (v *StatelessBlockValidator) CreateReadyValidationEntry(ctx context.Context, header *types.Header) (*validationEntry, error) {
	...
	msgIndex := fogutil.BlockNumberToMessageCount(blockNum, v.genesisBlockNum) - 1
