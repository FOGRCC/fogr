...
func (s *Staker) Start(ctxIn context.Context) {
	...
	s.CallIteratively(func(ctx context.Context) (returningWait time.Duration) {
		...
		fogTx, err := s.Act(ctx)
		if err == nil && fogTx != nil {
			_, err = s.l1Reader.WaitForTxApproval(ctx, fogTx)
			err = errors.Wrap(err, "error waiting for tx receipt")
			if err == nil {
				log.Info("successfully executed staker transaction", "hash", fogTx.Hash())
			}
		}
		if err == nil {
			backoff = time.Second
			if fogTx != nil && !s.wallet.CanBatchTxs() {
...
func (s *Staker) Act(ctx context.Context) (*types.Transaction, error) {
	...
	if shouldResolveNodes {
		fogTx, err := s.resolveTimedOutChallenges(ctx)
		if err != nil {
