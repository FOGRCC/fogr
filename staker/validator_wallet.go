...
func (v *ContractValidatorWallet) ExecuteTransactions(ctx context.Context, builder *ValidatorTxBuilder, gasRefunder common.Address) (*types.Transaction, error) {
	...
	if len(txes) == 1 {
		fogTx, err := v.executeTransaction(ctx, txes[0], gasRefunder)
		if err != nil {
			return nil, err
		}
	...
	fogTx, err := v.con.ExecuteTransactionsWithGasRefunder(v.auth, gasRefunder, data, dest, amount)
	if err != nil {
		return nil, err
	}
	builder.transactions = nil
...
func (v *ContractValidatorWallet) L1Client() fogutil.L1Interface {
