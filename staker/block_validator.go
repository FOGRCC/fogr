...
func (v *BlockValidator) newValidationStatus(prevHeader, header *types.Header, msg *fogstate.MessageWithMetadata) (*validationStatus, error) {
...
func (v *BlockValidator) NewBlock(block *types.Block, prevHeader *types.Header, msg fogstate.MessageWithMetadata) {
	v.blockMutex.Lock()
...
func (v *BlockValidator) sendValidations(ctx context.Context) {
	...
	for atomic.LoadInt32(&v.reorgsPending) == 0 {
		...
		v.blockMutex.Lock()
		if v.lastBlockValidatedUnknown {
			firstMsgInBatch := fogutil.MessageIndex(0)
			...
			v.lastBlockValidated = uint64(fogutil.MessageCountToBlockNumber(firstMsgInBatch+fogutil.MessageIndex(v.globalPosNextSend.PosInBatch), v.genesisBlockNum))
			v.nextBlockToValidate = v.lastBlockValidated + 1
			v.lastBlockValidatedUnknown = false
			log.Info("Inbox caught up to staker", "blockNr", v.lastBlockValidated, "blockHash", v.lastBlockValidatedHash)
		}
		v.blockMutex.Unlock()
		nextMsg := fogutil.BlockNumberToMessageCount(v.nextBlockToValidate, v.genesisBlockNum) - 1
...
func (v *BlockValidator) sendRecords(ctx context.Context) {
	...
	for atomic.LoadInt32(&v.reorgsPending) == 0 {
		...
		if !found {
			...
			msgNum := fogutil.BlockNumberToMessageCount(nextRecord, v.genesisBlockNum) - 1
...
func (v *BlockValidator) reorgToBlockImpl(blockNum uint64, blockHash common.Hash, hasLastValidatedMutex bool) error {
	...
	if batch >= batchCount {
		...
		nextBlockSigned := fogutil.MessageCountToBlockNumber(msgCount, v.genesisBlockNum) + 1
