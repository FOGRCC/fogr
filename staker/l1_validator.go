...
func (v *L1Validator) blockNumberFromGlobalState(gs validator.GoGlobalState) (int64, bool, error) {
	var batchHeight fogutil.MessageIndex
	if gs.Batch > 0 {
		var err error
	...
	if gs.PosInBatch > 0 {
		...
		if gs.PosInBatch >= uint64(nextBatchHeight-batchHeight) {
			...
			return fogutil.MessageCountToBlockNumber(nextBatchHeight, v.genesisBlockNumber), true, nil
		}
	}

	return fogutil.MessageCountToBlockNumber(batchHeight+fogutil.MessageIndex(gs.PosInBatch), v.genesisBlockNumber), false, nil
}
...
func (v *L1Validator) generateNodeAction(ctx context.Context, stakerInfo *OurStakerInfo, strategy StakerStrategy, makeAssertionInterval time.Duration) (nodeAction, bool, error) {
	...
	if v.blockValidator != nil {
		...
		if localBatchCount > 0 {
			...
			lastBatchBlock := uint64(fogutil.MessageCountToBlockNumber(messageCount, v.genesisBlockNumber))
...
func (v *L1Validator) createNewNodeAction(
	...
	for i := localBatchCount - 1; i+1 >= minBatchCount && i > 0; i-- {
		...
		// Must be non-negative as a batch must contain at least one message
		lastBlockNum := uint64(fogutil.MessageCountToBlockNumber(batchMessageCount, v.genesisBlockNumber))
		prevBlockNum := uint64(fogutil.MessageCountToBlockNumber(prevBatchMessageCount, v.genesisBlockNumber))
