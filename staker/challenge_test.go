...
func CreateChallenge(
	...
	inputMachine server_fog.MachineInterface,
...
func runChallengeTest(
	t *testing.T,
	baseMachine *server_fog.fogitratorMachine,
	incorrectMachine server_fog.MachineInterface,
	...
	var asserterMachine, challengerMachine server_fog.MachineInterface
	...
	asserterRun, err := server_fog.NewExecutionRun(ctx,
		func(context.Context) (server_fog.MachineInterface, error) { return asserterMachine, nil },
		&server_fog.DefaultMachineCacheConfig)
	Require(t, err)
	...
	challengerRun, err := server_fog.NewExecutionRun(ctx,
		func(context.Context) (server_fog.MachineInterface, error) { return challengerMachine, nil },
		&server_fog.DefaultMachineCacheConfig)
	Require(t, err)
...
func createBaseMachine(t *testing.T, wasmname string, wasmModules []string) *server_fog.fogitratorMachine {
	_, filename, _, _ := runtime.Caller(0)
	wasmDir := path.Join(path.Dir(filename), "../fogitrator/prover/test-cases/")
	...
	machine, err := server_fog.LoadSimpleMachine(wasmPath, modulePaths)
	Require(t, err)
...
func TestChallengeToOSP(t *testing.T) {
	...
	IncorrectMachine := server_fog.NewIncorrectMachine(machine, 200)
...
func TestChallengeToErroredOSP(t *testing.T) {
	...
	IncorrectMachine := server_fog.NewIncorrectMachine(machine, 10)
	runChallengeTest(t, machine, IncorrectMachine, false, false, 0)
}
