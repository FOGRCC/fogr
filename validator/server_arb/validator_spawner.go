...
type fogitratorSpawnerConfig struct {
...
type fogitratorSpawnerConfigFecher func() *fogitratorSpawnerConfig

var DefaultfogitratorSpawnerConfig = fogitratorSpawnerConfig{
	Workers:        0,
...
func fogitratorSpawnerConfigAddOptions(prefix string, f *flag.FlagSet) {
	f.Int(prefix+".workers", DefaultfogitratorSpawnerConfig.Workers, "number of concurrent validation threads")
	f.Duration(prefix+".execution-run-timeout", DefaultfogitratorSpawnerConfig.ExecRunTimeout, "timeout before discarding execution run")
	f.String(prefix+".output-path", DefaultfogitratorSpawnerConfig.OutputPath, "path to write machines to")
	MachineCacheConfigConfigAddOptions(prefix+".execution", f)
}

func DefaultfogitratorSpawnerConfigFetcher() *fogitratorSpawnerConfig {
	return &DefaultfogitratorSpawnerConfig
}

type fogitratorSpawner struct {
	stopwaiter.StopWaiter
	count         int32
	locator       *server_common.MachineLocator
	machineLoader *fogMachineLoader
	config        fogitratorSpawnerConfigFecher
}
...
func NewfogitratorSpawner(locator *server_common.MachineLocator, config fogitratorSpawnerConfigFecher) (*fogitratorSpawner, error) {
	// TODO: preload machines
	spawner := &fogitratorSpawner{
		locator:       locator,
		machineLoader: NewfogMachineLoader(&DefaultfogitratorMachineConfig, locator),
		config:        config,
	}
	return spawner, nil
}

func (s *fogitratorSpawner) Start(ctx_in context.Context) error {
	s.StopWaiter.Start(ctx_in, s)
	return nil
}

func (s *fogitratorSpawner) LatestWasmModuleRoot() (common.Hash, error) {
	return s.locator.LatestWasmModuleRoot(), nil
}

func (s *fogitratorSpawner) Name() string {
	return "fogitrator"
}

func (v *fogitratorSpawner) loadEntryToMachine(ctx context.Context, entry *validator.ValidationInput, mach *fogitratorMachine) error {
...
func (v *fogitratorSpawner) execute(
...
func (v *fogitratorSpawner) Launch(entry *validator.ValidationInput, moduleRoot common.Hash) validator.ValidationRun {
...
func (v *fogitratorSpawner) Room() int {
...
//nolint:gosec
func (v *fogitratorSpawner) WriteToFile(input *validator.ValidationInput, expOut validator.GoGlobalState, moduleRoot common.Hash) error {
...
func (v *fogitratorSpawner) CreateExecutionRun(wasmModuleRoot common.Hash, input *validator.ValidationInput) (validator.ExecutionRun, error) {
