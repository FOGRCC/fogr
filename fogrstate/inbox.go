...
import (
	...
	"github.com/FOGRCC/fogr/zeroheavy"
)

var uniquifyingPrefix = []byte("FOGR Feed:")
...
type InboxBackend interface {
	...
	ReadDelayedInbox(seqNum uint64) (*fogos.L1IncomingMessage, error)
}

type MessageWithMetadata struct {
	Message             *fogos.L1IncomingMessage `json:"message"`
	DelayedMessagesRead uint64                   `json:"delayedMessagesRead"`
}

var EmptyTestMessageWithMetadata = MessageWithMetadata{
	Message: &fogos.EmptyTestIncomingMessage,
}
...
var TestMessageWithMetadataAndRequestId = MessageWithMetadata{
	Message: &fogos.TestIncomingMessageWithRequestId,
}

func (m *MessageWithMetadata) Hash(sequenceNumber fogutil.MessageIndex, chainId uint64) (common.Hash, error) {
...
func parseSequencerMessage(ctx context.Context, batchNum uint64, data []byte, dasReader DataAvailabilityReader, keysetValidationMode KeysetValidationMode) (*sequencerMessage, error) {
	...
	if len(payload) > 0 && IsBrotliMessageHeaderByte(payload[0]) {
		decompressed, err := fogcompress.Decompress(payload[1:], MaxDecompressedLen)
		if err == nil {
...
var InvalidL1Message = &fogos.L1IncomingMessage{
	Header: &fogos.L1IncomingMessageHeader{
		Kind: fogos.L1MessageType_Invalid,
	},
	L2msg: []byte{},
}
...
func (r *inboxMultiplexer) getNextMsg() (*MessageWithMetadata, error) {
	...
	if kind == BatchSegmentKindL2Message || kind == BatchSegmentKindL2MessageBrotli {
		...
		if kind == BatchSegmentKindL2MessageBrotli {
			decompressed, err := fogcompress.Decompress(segment, fogos.MaxL2MessageSize)
			if err != nil {
