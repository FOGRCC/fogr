...
var PackfogRetryableTxRedeem func(...interface{}) ([]byte, error)
...
func init() {
	...
	InverseAddressAliasOffset = fogmath.BigSub(new(big.Int).Lsh(big.NewInt(1), 160), AddressAliasOffset)
	...
	ParseRedeemScheduledLog = logParser(precompilesgen.fogRetryableTxABI, "RedeemScheduled")
	ParseL2ToL1TxLog = logParser(precompilesgen.fogSysABI, "L2ToL1Tx")
	ParseL2ToL1TransactionLog = logParser(precompilesgen.fogSysABI, "L2ToL1Transaction")

	acts := precompilesgen.fogosActsABI
	...
	PackfogRetryableTxRedeem, _ = callParser(precompilesgen.fogRetryableTxABI, "redeem")
}
...
func DoesTxTypeAlias(txType *byte) bool {
	...
	switch *txType {
	case types.FOGUnsignedTxType:
		fallthrough
	case types.FOGContractTxType:
		fallthrough
	case types.FOGRetryTxType:
		return true
	}
	return false
}
...
func TxTypeHasPosterCosts(txType byte) bool {
	...
	case types.FOGRetryTxType:
		fallthrough
	case types.FOGInternalTxType:
		fallthrough
	case types.FOGSubmitRetryableTxType:
		return false
	}
	return true
}
