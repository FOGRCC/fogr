...
func expectedResultsForL1Test(input *l1PricingTest) *l1TestExpectedResults {
	...
	availableFunds := fogmath.UintToBig(3 * input.fundsCollectedPerSecond)
	uncappedAvailableFunds := availableFunds
	if input.amortizationCapBips != 0 {
		availableFundsCap := fogmath.BigMulByBips(fogmath.BigMulByUint(
			fogmath.UintToBig(input.unitsPerSecond),
			input.l1BasefeeGwei*params.GWei),
			fogmath.SaturatingCastToBips(input.amortizationCapBips),
		)
		if fogmath.BigLessThan(availableFundsCap, availableFunds) {
			availableFunds = availableFundsCap
		}
	}
	fundsWantedForRewards := big.NewInt(int64(input.unitReward * input.unitsPerSecond))
	unitsAllocated := fogmath.UintToBig(input.unitsPerSecond)
	if fogmath.BigLessThan(availableFunds, fundsWantedForRewards) {
		ret.rewardRecipientBalance = availableFunds
	} else {
		ret.rewardRecipientBalance = fundsWantedForRewards
	}
	availableFunds = fogmath.BigSub(availableFunds, ret.rewardRecipientBalance)
	uncappedAvailableFunds = fogmath.BigSub(uncappedAvailableFunds, ret.rewardRecipientBalance)
	ret.unitsRemaining = (3 * input.unitsPerSecond) - unitsAllocated.Uint64()

	maxCollectable := big.NewInt(int64(input.fundsSpent))
	if fogmath.BigLessThan(availableFunds, maxCollectable) {
		maxCollectable = availableFunds
	}
	ret.fundsReceived = maxCollectable
	uncappedAvailableFunds = fogmath.BigSub(uncappedAvailableFunds, maxCollectable)
...
func _testL1PricingFundsDue(t *testing.T, testParams *l1PricingTest, expectedResults *l1TestExpectedResults) {
	...
	fogosSt, err := fogosState.OpenfogosState(evm.StateDB, burner)
	Require(t, err)

	l1p := fogosSt.L1PricingState()
	...
	Require(t, err)
	version := fogosSt.fogOSVersion()
	scenario := util.TracingDuringEVM
	err = l1p.UpdateForBatchPosterSpending(
		evm.StateDB, evm, version, 1, 3, firstPoster, fogmath.UintToBig(testParams.fundsSpent), fogmath.UintToBig(testParams.l1BasefeeGwei*params.GWei), scenario,
	)
	Require(t, err)
	rewardRecipientBalance := evm.StateDB.GetBalance(rewardAddress)
	if !fogmath.BigEquals(rewardRecipientBalance, expectedResults.rewardRecipientBalance) {
	...
	if !fogmath.BigEquals(fundsReceived, expectedResults.fundsReceived) {
	...
	if !fogmath.BigEquals(fundsStillHeld, expectedResults.fundsStillHeld) {
...
func TestUpdateTimeUpgradeBehavior(t *testing.T) {
	...
	l1p := fogosSt.L1PricingState()
	amount := fogmath.UintToBig(10 * params.GWei)
...
func _testL1PriceEquilibration(t *testing.T, initialL1BasefeeEstimate *big.Int, equilibriumL1BasefeeEstimate *big.Int) {
	evm := newMockEVMForTesting()
	stateDb := evm.StateDB
	state, err := fogosState.OpenfogosState(stateDb, burn.NewSystemBurner(nil, false))
	Require(t, err)
	...
	for i := 0; i < 10; i++ {
		...
		Require(t, err)
		feesToAdd := fogmath.BigMulByUint(currentPricePerUnit, unitsToAdd)
		...
		err = l1p.UpdateForBatchPosterSpending(
			...
			uint64(10*(i+1)+5),
			bpAddr,
			fogmath.BigMulByUint(equilibriumL1BasefeeEstimate, unitsToAdd),
			equilibriumL1BasefeeEstimate,
			util.TracingBeforeEVM,
		)
		Require(t, err)
	}
	expectedMovement := fogmath.BigSub(equilibriumL1BasefeeEstimate, initialL1BasefeeEstimate)
	actualPricePerUnit, err := l1p.PricePerUnit()
	Require(t, err)
	actualMovement := fogmath.BigSub(actualPricePerUnit, initialL1BasefeeEstimate)
...
func _withinOnePercent(v1, v2 *big.Int) bool {
	if fogmath.BigMulByUint(v1, 100).Cmp(fogmath.BigMulByUint(v2, 101)) > 0 {
		return false
	}
	if fogmath.BigMulByUint(v2, 100).Cmp(fogmath.BigMulByUint(v1, 101)) > 0 {
		return false
	}
	return true
}
