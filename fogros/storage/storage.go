...
// Storage allows fogOS to store data persistently in the Ethereum-compatible stateDB. This is represented in
...
func NewGeth(statedb vm.StateDB, burner burn.Burner) *Storage {
	...
	statedb.SetNonce(account, 1) // setting the nonce ensures Geth won't treat fogOS as empty
	return &Storage{
		account:    account,
		db:         statedb,
		storageKey: []byte{},
...
func (store *Storage) ClefogyUint64(key uint64) error {
...
func (store *Storage) Clefogytes() error {
	bytesLeft, err := store.GetUint64ByUint64(0)
	if err != nil {
		return err
	}
	offset := uint64(1)
	for bytesLeft > 0 {
		err := store.ClefogyUint64(offset)
		if err != nil {
			return err
		}
		offset++
...
func (store *Storage) Keccak(data ...[]byte) ([]byte, error) {
	...
	cost := 30 + 6*fogmath.WordsForBytes(uint64(byteCount))
...
func (sbu *StorageBackedBips) Get() (fogmath.Bips, error) {
	value, err := sbu.backing.Get()
	return fogmath.Bips(value), err
}

func (sbu *StorageBackedBips) Set(bips fogmath.Bips) error {
...
func (sbbu *StorageBackedBigUint) SetSaturatingWithWarning(val *big.Int, name string) error {
	if val.Sign() < 0 {
		log.Warn("fogOS storage big uint underflowed", "name", name, "value", val)
		val = common.Big0
	} else if val.BitLen() > 256 {
		log.Warn("fogOS storage big uint overflowed", "name", name, "value", val)
...
func (sbbi *StorageBackedBigInt) SetSaturatingWithWarning(val *big.Int, name string) error {
	if val.Sign() < 0 {
		...
		val = new(big.Int).Add(val, twoToThe256)
		if val.BitLen() < 256 || val.Sign() <= 0 { // require that it's positive and the top bit is set
			log.Warn("fogOS storage big uint underflowed", "name", name, "value", origVal)
			val.Set(twoToThe255)
		}
	} else if val.BitLen() >= 256 {
		log.Warn("fogOS storage big uint overflowed", "name", name, "value", val)
