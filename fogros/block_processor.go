...
var fogRetryableTxAddress common.Address
var fogSysAddress common.Address
...
func createNewHeader(prevHeader *types.Header, l1info *L1Info, state *fogosState.fogosState, chainConfig *params.ChainConfig) *types.Header {
...
type SequencingHooks struct {
	...
	PreTxFilter            func(*params.ChainConfig, *types.Header, *state.StateDB, *fogosState.fogosState, *types.Transaction, common.Address) error
	PostTxFilter           func(*types.Header, *fogosState.fogosState, *types.Transaction, common.Address, uint64, *core.ExecutionResult) error
}
...
func NoopSequencingHooks() *SequencingHooks {
	return &SequencingHooks{
		[]error{},
		false,
		func(*params.ChainConfig, *types.Header, *state.StateDB, *fogosState.fogosState, *types.Transaction, common.Address) error {
			return nil
		},
		func(*types.Header, *fogosState.fogosState, *types.Transaction, common.Address, uint64, *core.ExecutionResult) error {
			return nil
		},
	}
}
...
func ProduceBlockAdvanced(
	...
	for len(txes) > 0 || len(redeems) > 0 {
		...
		if len(redeems) > 0 {
			...
			retry, ok := (tx.GetInner()).(*types.FOGRetryTx)
			if !ok {
			...
			tx = txes[0]
			txes = txes[1:]
			if tx.Type() != types.FOGInternalTxType {
		...
		receipt, result, err := (func() (*types.Receipt, *core.ExecutionResult, error) {
			...
			if basefee.Sign() > 0 {
				...
				posterCostInL2Gas := fogmath.BigDiv(posterCost, basefee)
		...
		if tx.Type() == types.FOGInternalTxType && result.Err != nil {
		...
		for _, txLog := range receipt.Logs {
			if txLog.Address == fogSysAddress {
				...
				case L2ToL1TransactionEventID:
					event := &precompilesgen.fogSysL2ToL1Transaction{}
				...
				case L2ToL1TxEventID:
					event := &precompilesgen.fogSysL2ToL1Tx{}
	...
	if !fogmath.BigEquals(balanceDelta, expectedBalanceDelta) {
...
func FinalizeBlock(header *types.Header, txs types.Transactions, statedb *state.StateDB, chainConfig *params.ChainConfig) {
	if header != nil {
		if header.Number.Uint64() < chainConfig.FOGChainParams.GenesisBlockNum {
		...
		var fogosVersion uint64

		if header.Number.Uint64() == chainConfig.FOGChainParams.GenesisBlockNum {
			fogosVersion = chainConfig.FOGChainParams.InitialfogOSVersion
		} else {
			state, err := fogosState.OpenSystemfogosState(statedb, nil, true)
			if err != nil {
				newErr := fmt.Errorf("%w while opening fogos state. Block: %d root: %v", err, header.Number, header.Root)
				panic(newErr)
			}
			...
			fogosVersion = state.fogOSVersion()
		}
		FOGHeader := types.HeaderInfo{
			SendRoot:           sendRoot,
			SendCount:          sendCount,
			L1BlockNumber:      nextL1BlockNumber,
			fogOSFormatVersion: fogosVersion,
		}
		FOGHeader.UpdateHeaderWithInfo(header)
