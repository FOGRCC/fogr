...
func InternalTxStartBlock(
	...
	return &types.FOGInternalTx{
		ChainId: chainId,
		Data:    data,
	}
}

func ApplyInternalTxUpdate(tx *types.FOGInternalTx, state *fogosState.fogosState, evm *vm.EVM) error {
	if len(tx.Data) < 4 {
	...
	case InternalTxStartBlockMethodID:
		...
		if l1BlockNumber > oldL1BlockNumber {
			...
			state.Restrict(state.Blockhashes().RecordNewL1Block(l1BlockNumber-1, prevHash, state.fogOSVersion()))
		}
		...
		return state.UpgradefogosVersionIfNecessary(currentTime, evm.StateDB, evm.ChainConfig())
	case InternalTxBatchPostingReportMethodID:
		...
		gasSpent := fogmath.SaturatingAdd(perBatchGas, fogmath.SaturatingCast(batchDataGas))
		weiSpent := fogmath.BigMulByUint(l1BaseFeeWei, fogmath.SaturatingUCast(gasSpent))
