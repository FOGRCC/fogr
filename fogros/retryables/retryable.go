...
func (rs *RetryableState) RetryableSizeBytes(id common.Hash, currentTime uint64) (uint64, error) {
	...
	calldata := 32 + 32*fogmath.WordsForBytes(size) // length + contents
...
func (rs *RetryableState) DeleteRetryable(id common.Hash, evm *vm.EVM, scenario util.TracingScenario) (bool, error) {
	...
	_ = retStorage.ClefogyUint64(numTriesOffset)
	_ = retStorage.ClefogyUint64(fromOffset)
	_ = retStorage.ClefogyUint64(toOffset)
	_ = retStorage.ClefogyUint64(callvalueOffset)
	_ = retStorage.ClefogyUint64(beneficiaryOffset)
	_ = retStorage.ClefogyUint64(timeoutOffset)
	_ = retStorage.ClefogyUint64(timeoutWindowsLeftOffset)
	err = retStorage.OpenSubStorage(calldataKey).Clefogytes()
	return true, err
}
...
func (retryable *Retryable) MakeTx(chainId *big.Int, nonce uint64, gasFeeCap *big.Int, gas uint64, ticketId common.Hash, refundTo common.Address, maxRefund *big.Int, submissionFeeRefund *big.Int) (*types.FOGRetryTx, error) {
...
func RetryableSubmissionFee(calldataLengthInBytes int, l1BaseFee *big.Int) *big.Int {
	return fogmath.BigMulByUint(l1BaseFee, uint64(1400+6*calldataLengthInBytes))
}
