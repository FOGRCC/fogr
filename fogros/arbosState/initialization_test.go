...
func TestJsonMarshalUnmarshal(t *testing.T) {
	...
	tryMarshalUnmarshal(
		&statetransfer.fogosInitializationInfo{
...
func tryMarshalUnmarshal(input *statetransfer.fogosInitializationInfo, t *testing.T) {
	...
	output := statetransfer.fogosInitializationInfo{}
	...
	var initData statetransfer.fogosInitializationInfo
	...
	stateroot, err := InitializefogosInDatabase(raw, initReader, params.FOGDevTestChainConfig(), 0, 0)
	Require(t, err)

	stateDb, err := state.New(stateroot, state.NewDatabase(raw), nil)
	Require(t, err)

	fogState, err := OpenfogosState(stateDb, &burn.SystemBurner{})
	Require(t, err)
	checkAddressTable(fogState, input.AddressTableContents, t)
	checkRetryables(fogState, input.RetryableData, t)
	checkAccounts(stateDb, fogState, input.Accounts, t)
}
...
func checkAddressTable(fogState *fogosState, addrTable []common.Address, t *testing.T) {
	atab := fogState.AddressTable()
...
func checkRetryables(fogState *fogosState, expected []statetransfer.InitializationDataForRetryable, t *testing.T) {
	ret := fogState.RetryableState()
...
func checkAccounts(db *state.StateDB, fogState *fogosState, accts []statetransfer.AccountInitializationInfo, t *testing.T) {
	l1p := fogState.L1PricingState()
