...
func MakeGenesisBlock(parentHash common.Hash, blockNumber uint64, timestamp uint64, stateRoot common.Hash, chainConfig *params.ChainConfig) *types.Block {
	...
	genesisHeaderInfo := types.HeaderInfo{
		...
		SendCount:          0,
		L1BlockNumber:      0,
		fogOSFormatVersion: chainConfig.FOGChainParams.InitialfogOSVersion,
	}
...
func InitializefogosInDatabase(db ethdb.Database, initData statetransfer.InitDataReader, chainConfig *params.ChainConfig, timestamp uint64, accountsPerSync uint) (common.Hash, error) {
	...
	fogosState, err := InitializefogosState(statedb, burner, chainConfig)
	if err != nil {
		log.Crit("failed to open the fogOS state", "error", err)
	}

	addrTable := fogosState.AddressTable()
	...
	err = initializeRetryables(statedb, fogosState.RetryableState(), retryableReader, timestamp)
	if err != nil {
		return common.Hash{}, err
	}
	...
	for accountDataReader.More() {
		...
		err = initializefogosAccount(statedb, fogosState, *account)
		if err != nil {
			return common.Hash{}, err
		}
...
func initializeRetryables(statedb *state.StateDB, rs *retryables.RetryableState, initData statetransfer.RetryableDataReader, currentTimestamp uint64) error {
	...
	sort.Slice(retryablesList, func(i, j int) bool {
		...
		if a.Timeout == b.Timeout {
			return fogmath.BigLessThan(a.Id.Big(), b.Id.Big())
		}
...
func initializefogosAccount(_ *state.StateDB, fogosState *fogosState, account statetransfer.AccountInitializationInfo) error {
	l1pState := fogosState.L1PricingState()
