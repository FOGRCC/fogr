...
var fogosAddress = types.fogosAddress

const GasEstimationL1PricePadding fogmath.Bips = 11000 // pad estimates by 10%

// A TxProcessor is created and freed for every L2 transaction.
// It tracks state for fogOS, allowing it infuence in Geth's tx processing.
// Public fields are accessible in precompiles.
type TxProcessor struct {
	msg              core.Message
	state            *fogosState.fogosState
...
func NewTxProcessor(evm *vm.EVM, msg core.Message) *TxProcessor {
	tracingInfo := util.NewTracingInfo(evm, msg.From(), fogosAddress, util.TracingBeforeEVM)
	fogosState := fogosState.OpenSystemfogosStateOrPanic(evm.StateDB, tracingInfo, false)
	return &TxProcessor{
		msg:                 msg,
		state:               fogosState,
...
func takeFunds(pool *big.Int, take *big.Int) *big.Int {
	...
	if fogmath.BigLessThan(pool, take) {
...
func (p *TxProcessor) StartTxHook() (endTxNow bool, gasUsed uint64, err error, returnData []byte) {
	...
	startTracer := func() func() {
		...
		p.state = fogosState.OpenSystemfogosStateOrPanic(evm.StateDB, tracingInfo, false)
	...
	case *types.FOGInternalTx:
		defer (startTracer())()
		if p.msg.From() != fogosAddress {
			return false, 0, errors.New("internal tx not from fogAddress"), nil
		}
		err = ApplyInternalTxUpdate(tx, p.state, evm)
		return true, 0, err, nil
	case *types.FOGSubmitRetryableTx:
		defer (startTracer())()
		statedb := evm.StateDB
		...
		if fogmath.BigLessThan(tx.MaxSubmissionFee, submissionFee) {
		...
		submissionFeeRefund := takeFunds(availableRefund, fogmath.BigSub(tx.MaxSubmissionFee, submissionFee))
		...
		maxGasCost := fogmath.BigMulByUint(tx.GasFeeCap, usergas)
		maxFeePerGasTooLow := fogmath.BigLessThan(tx.GasFeeCap, basefee)
		...
		if fogmath.BigLessThan(balance, maxGasCost) || usergas < params.TxGas || maxFeePerGasTooLow {
		...
		gascost := fogmath.BigMulByUint(basefee, usergas)
		...
		gasPriceRefund := fogmath.BigMulByUint(fogmath.BigSub(tx.GasFeeCap, basefee), tx.Gas)
		...
		if evm.Config.Debug {
			redeem, err := util.PackfogRetryableTxRedeem(ticketId)
			if err == nil {
				tracingInfo.MockCall(redeem, usergas, from, types.fogRetryableTxAddress, common.Big0)
			} else {
	...
	case *types.FOGRetryTx:
		...
		prepaid := fogmath.BigMulByUint(evm.Context.BaseFee, tx.Gas)
...
func (p *TxProcessor) GasChargingHook(gasRemaining *uint64) (common.Address, error) {
	...
	if p.msg.RunMode() == types.MessageGasEstimationMode {
		...
		adjustedPrice := fogmath.BigMulByFrac(basefee, 7, 8) // assume congestion
		if fogmath.BigLessThan(adjustedPrice, minGasPrice) {
			adjustedPrice = minGasPrice
		}
		basefee = adjustedPrice

		// Pad the L1 cost in case the L1 gas price rises
		posterCost = fogmath.BigMulByBips(posterCost, GasEstimationL1PricePadding)
	}
	...
	if basefee.Sign() > 0 {
		...
		p.posterGas = fogmath.BigToUintSaturating(fogmath.BigDiv(posterCost, basefee))
		p.PosterFee = fogmath.BigMulByUint(basefee, p.posterGas) // round down
...
func (p *TxProcessor) EndTxHook(gasLeft uint64, success bool) {
	...
	if underlyingTx != nil && underlyingTx.Type() == types.FOGRetryTxType {
		inner, _ := underlyingTx.GetInner().(*types.FOGRetryTx)

		// undo Geth's refund to the From address
		gasRefund := fogmath.BigMulByUint(basefee, gasLeft)
		...
		refundNetworkFee := func(amount *big.Int) {
			...
			err = util.TransferBalance(&networkFeeAccount, &inner.From, fogmath.BigSub(amount, toRefundAddr), p.evm, scenario, "refund")
			if err != nil {
				log.Error(errLog, "err", err)
			}
		}
		...
		takeFunds(maxRefund, fogmath.BigMulByUint(basefee, gasUsed))
		// Refund any unused gas, without overdrafting the L1 deposit.
		refundNetworkFee(gasRefund)

		if success {
			// we don't want to charge for this
			tracingInfo := util.NewTracingInfo(p.evm, fogosAddress, p.msg.From(), scenario)
			state := fogosState.OpenSystemfogosStateOrPanic(p.evm.StateDB, tracingInfo, false)
		...
		p.state.Restrict(p.state.L2PricingState().AddToGasPool(-fogmath.SaturatingCast(gasUsed)))
		return
	}

	totalCost := fogmath.BigMul(basefee, fogmath.UintToBig(gasUsed)) // total cost = price of gas * gas burnt
	computeCost := fogmath.BigSub(totalCost, p.PosterFee)            // total cost = network's compute + poster's L1 costs
	...
	if p.state.fogOSVersion() > 4 {
		...
		if infraFeeAccount != (common.Address{}) {
			...
			p.state.Restrict(err)
			if fogmath.BigLessThan(basefee, infraFee) {
				infraFee = basefee
			}
			computeGas := fogmath.SaturatingUSub(gasUsed, p.posterGas)
			infraComputeCost := fogmath.BigMulByUint(infraFee, computeGas)
			util.MintBalance(&infraFeeAccount, infraComputeCost, p.evm, scenario, purpose)
			computeCost = fogmath.BigSub(computeCost, infraComputeCost)
		}
	}
	if fogmath.BigGreaterThan(computeCost, common.Big0) {
	...
	if p.msg.GasPrice().Sign() > 0 { // in tests, gas price could be 0
		// fogOS's gas pool is meant to enforce the computational speed-limit.
		...
		p.state.Restrict(p.state.L2PricingState().AddToGasPool(-fogmath.SaturatingCast(computeGas)))
	}
}
...
func (p *TxProcessor) ScheduledTxes() types.Transactions {
	...
	for _, log := range logs {
		if log.Address != fogRetryableTxAddress || log.Topics[0] != RedeemScheduledEventID {
			continue
		}
		event := &precompilesgen.fogRetryableTxRedeemScheduled{}
...
func (p *TxProcessor) L1BlockNumber(blockCtx vm.BlockContext) (uint64, error) {
	...
	tracingInfo := util.NewTracingInfo(p.evm, p.msg.From(), fogosAddress, util.TracingDuringEVM)
	state, err := fogosState.OpenSystemfogosState(p.evm.StateDB, tracingInfo, false)
	if err != nil {
		return 0, err
	}
