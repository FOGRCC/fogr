...
type L1PricingState struct {
	...
	lastSurplus          storage.StorageBackedBigInt  // introduced in fogOS version 2
	perBatchGasCost      storage.StorageBackedInt64   // introduced in fogOS version 3
	amortizedCostCapBips storage.StorageBackedUint64  // in basis points; introduced in fogOS version 3
...
var InitialEquilibrationUnitsV0 = fogmath.UintToBig(60 * params.TxDataNonZeroGasEIP2028 * 100000)
var InitialEquilibrationUnitsV6 = fogmath.UintToBig(params.TxDataNonZeroGasEIP2028 * 10000000)
...
func (ps *L1PricingState) SetLastSurplus(val *big.Int, fogosVersion uint64) error {
	if fogosVersion < 7 {
...
func (ps *L1PricingState) UpdateForBatchPosterSpending(
	...
	if fogosVersion < 10 {
		return ps._preversion10_UpdateForBatchPosterSpending(statedb, evm, fogosVersion, updateTime, currentTime, batchPoster, weiSpent, l1Basefee, scenario)
	}
	...
	if unitsAllocated > 0 {
		...
		if err := ps.SetLastSurplus(surplus, fogosVersion); err != nil {
			return err
		}
...
var randV = fogmath.BigMulByUint(params.FOGOneChainConfig().ChainID, 3)
...
func (ps *L1PricingState) PosterDataCost(message core.Message, poster common.Address) (*big.Int, uint64) {
	...
	units := ps.getPosterUnitsWithoutCache(tx, poster)
	units = fogmath.UintMulByBips(units+estimationPaddingUnits, fogmath.OneInBips+estimationPaddingBasisPoints)
	pricePerUnit, _ := ps.PricePerUnit()
	return am.BigMulByUint(pricePerUnit, units), units
}

func byteCountAfterBrotli0(input []byte) (uint64, error) {
	compressed, err := fogcompress.CompressFast(input)
	if err != nil {
		return 0, err
	}
