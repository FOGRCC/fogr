...
// To avoid creating new RPC methods for client-side tooling, fogr Geth's InterceptRPCMessage() hook provides
...
func (n NodeInterface) FOGGenesisBlock(c ctx) (huge, error) {
	block := n.backend.ChainConfig().FOGChainParams.GenesisBlockNum
	return fogmath.UintToBig(block), nil
}

func (n NodeInterface) FindBatchContainingBlock(c ctx, evm mech, blockNum uint64) (uint64, error) {
	node, err := fogNodeFromNodeInterfaceBackend(n.backend)
	if err != nil {
		return 0, err
	}
...
func (n NodeInterface) GetL1Confirmations(c ctx, evm mech, blockHash bytes32) (uint64, error) {
	...
	bc := node.fogInterface.BlockChain()
	...
	if latestL1Block < meta.L1Block || fogutil.BlockNumberToMessageCount(blockNum, genesis) > meta.MessageCount {
		return 0, nil
	}
...
func (n NodeInterface) EstimateRetryableTicket(
	...
	submitTx := &types.FOGSubmitRetryableTx{
		ChainId:          nil,
		RequestId:        hash{},
	...
	// FOGSubmitRetryableTx is unsigned so the following won't panic
	msg, err := types.NewTx(submitTx).AsMessage(types.NewFOGSigner(nil), nil)
...
func (n NodeInterface) ConstructOutboxProof(c ctx, evm mech, size, leaf uint64) (bytes32, bytes32, []bytes32, error) {
	...
	balanced := size == fogmath.NextPowerOf2(size)/2
	treeLevels := int(fogmath.Log2ceil(size)) // the # of levels in the tree
	...
	search = func(lo, hi uint64, find []merkletree.LevelAndLeaf) {
		...
		if lo == hi {
			...
			for _, tx := range all {
				for _, log := range tx {
					if log.Address != types.fogSysAddress {
						// log not produced by fogOS
						continue
					}
...
func (n NodeInterface) GasEstimateL1Component(
	...
	feeForL1 = fogmath.BigMulByBips(feeForL1, fogos.GasEstimationL1PricePadding)
	gasForL1 := fogmath.BigDiv(feeForL1, baseFee).Uint64()
...
func (n NodeInterface) GasEstimateComponents(
	...
	totalRaw, err := FOGR.EstimateGas(context, backend, args, block, gasCap)
	if err != nil {
		return 0, 0, nil, nil, err
	}
...
func findBatchContainingBlock(node *fognode.Node, genesis uint64, block uint64) (uint64, error) {
	if block <= genesis {
		return 0, fmt.Errorf("%wblock %v is part of genesis", blockInGenesis, block)
	}
	pos := fogutil.BlockNumberToMessageCount(block, genesis) - 1
	...
	latestBlock := fogutil.MessageCountToBlockNumber(latestCount, genesis)
