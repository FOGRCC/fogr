...
func TestfogAggregatorBatchPosters(t *testing.T) {
	...
	bps, err := fogAggregator{}.GetBatchPosters(context, evm)
	Require(t, err)
	if len(bps) != 1 {
		Fail(t)
	}
	...
	Require(t, fogDebug{}.BecomeChainOwner(context, evm))
	Require(t, fogAggregator{}.AddBatchPoster(context, evm, addr))
