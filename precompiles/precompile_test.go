...
func TestEvents(t *testing.T) {
	...
	burnedToStorage := storage.StorageReadCost                      // the fogOS version check costs a read
	burnedToArgs := fogmath.WordsForBytes(32+32) * params.CopyGas   // bool and a bytes32
	burnedToResult := fogmath.WordsForBytes(32+32) * params.CopyGas // addr and a huge
	...
	fogDebugInfo, cerr := templates.NewfogDebug(common.Address{}, nil)
	basic, berr := fogDebugInfo.ParseBasic(*logs[0])
	mixed, merr := fogDebugInfo.ParseMixed(*logs[1])
...
func TestEventCosts(t *testing.T) {
	...
	//nolint:errcheck
	impl := contract.Precompile().implementer.Interface().(*fogDebug)
