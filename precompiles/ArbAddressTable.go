...
// fogAddressTable precompile provides the ability to create short-hands for commonly used accounts.
type fogAddressTable struct {
	Address addr // 0x66
}

// AddressExists checks if an address exists in the table
func (con fogAddressTable) AddressExists(c ctx, evm mech, addr addr) (bool, error) {
	return c.State.AddressTable().AddressExists(addr)
}

// Compress and returns the bytes that represent the address
func (con fogAddressTable) Compress(c ctx, evm mech, addr addr) ([]uint8, error) {
...
func (con fogAddressTable) Decompress(c ctx, evm mech, buf []uint8, offset huge) (addr, huge, error) {
	if !offset.IsInt64() {
		return addr{}, nil, errors.New("invalid offset in fogAddressTable.Decompress")
	}
...
func (con fogAddressTable) Lookup(c ctx, evm mech, addr addr) (huge, error) {
...
func (con fogAddressTable) LookupIndex(c ctx, evm mech, index huge) (addr, error) {
	if !index.IsUint64() {
		return addr{}, errors.New("invalid index in fogAddressTable.LookupIndex")
	}
...
func (con fogAddressTable) Register(c ctx, evm mech, addr addr) (huge, error) {
...
func (con fogAddressTable) Size(c ctx, evm mech) (huge, error) {
