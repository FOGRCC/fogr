...
type DebugPrecompile struct {
	precompile fogosPrecompile
}

// create a debug-only precompile wrapper
func debugOnly(address addr, impl fogosPrecompile) (addr, fogosPrecompile) {
...
func ownerOnly(address addr, impl fogosPrecompile, emit func(mech, bytes4, addr, []byte) error) (addr, fogosPrecompile) {
...
func (wrapper *OwnerPrecompile) Call(
	...
	state, err := fogosState.OpenfogosState(evm.StateDB, burner)
	if err != nil {
