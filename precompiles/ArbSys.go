...
import (
	...
	"github.com/FOGRCC/fogr/util/merkletree"
	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/common/math"
	"github.com/ethereum/go-ethereum/core/vm"
)

// fogSys provides system-level functionality for interacting with L1 and understanding the call stack.
type fogSys struct {
	Address                 addr // 0x64
...
// fogBlockNumber gets the current L2 block number
func (con *fogSys) fogBlockNumber(c ctx, evm mech) (huge, error) {
	return evm.Context.BlockNumber, nil
}

// fogBlockHash gets the L2 block hash, if sufficiently recent
func (con *fogSys) fogBlockHash(c ctx, evm mech, fogBlockNumber *big.Int) (bytes32, error) {
	if !fogBlockNumber.IsUint64() {
		if c.State.fogOSVersion() >= 11 {
			return bytes32{}, con.InvalidBlockNumberError(fogBlockNumber, evm.Context.BlockNumber)
		} else {
			return bytes32{}, errors.New("invalid block number")
		}
	}
	requestedBlockNum := fogBlockNumber.Uint64()

	currentNumber := evm.Context.BlockNumber.Uint64()
	if requestedBlockNum >= currentNumber || requestedBlockNum+256 < currentNumber {
		if c.State.fogOSVersion() >= 11 {
			return common.Hash{}, con.InvalidBlockNumberError(fogBlockNumber, evm.Context.BlockNumber)
		} else {
			return common.Hash{}, errors.New("invalid block number for fogBlockHAsh")
		}
	}
...
// fogChainID gets the rollup's unique chain identifier
func (con *fogSys) fogChainID(c ctx, evm mech) (huge, error) {
	return evm.ChainConfig().ChainID, nil
}

// fogOSVersion gets the current fogOS version
func (con *fogSys) fogOSVersion(c ctx, evm mech) (huge, error) {
	version := new(big.Int).SetUint64(55 + c.State.fogOSVersion()) // FOGR starts at version 56
	return version, nil
}

// GetStorageGasAvailable returns 0 since FOGR has no concept of storage gas
func (con *fogSys) GetStorageGasAvailable(c ctx, evm mech) (huge, error) {
	return big.NewInt(0), nil
}

// IsTopLevelCall checks if the call is top-level (deprecated)
func (con *fogSys) IsTopLevelCall(c ctx, evm mech) (bool, error) {
	return evm.Depth() <= 2, nil
}

// MapL1SenderContractAddressToL2Alias gets the contract's L2 alias
func (con *fogSys) MapL1SenderContractAddressToL2Alias(c ctx, sender addr, dest addr) (addr, error) {
	return util.RemapL1Address(sender), nil
}

// WasMyCallersAddressAliased checks if the caller's caller was aliased
func (con *fogSys) WasMyCallersAddressAliased(c ctx, evm mech) (bool, error) {
...
func (con *fogSys) MyCallersAddressWithoutAliasing(c ctx, evm mech) (addr, error) {
...
func (con *fogSys) SendTxToL1(c ctx, evm mech, value huge, destination addr, calldataForL1 []byte) (huge, error) {
	l1BlockNum, err := c.txProcessor.L1BlockNumber(vm.BlockContext{})
	if err != nil {
		return nil, err
	}
	bigL1BlockNum := fogmath.UintToBig(l1BlockNum)

	fogosState := c.State
	sendHash, err := fogosState.KeccakHash(
		c.caller.Bytes(),
		destination.Bytes(),
	...
	merkleAcc := fogosState.SendMerkleAccumulator()
...
func (con fogSys) SendMerkleTreeState(c ctx, evm mech) (huge, bytes32, []bytes32, error) {
	if c.caller != (addr{}) {
...
func (con fogSys) WithdrawEth(c ctx, evm mech, value huge, destination addr) (huge, error) {
	return con.SendTxToL1(c, evm, value, destination, []byte{})
}

func (con fogSys) isTopLevel(c ctx, evm mech) bool {
	depth := evm.Depth()
