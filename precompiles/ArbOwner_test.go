...
func TestfogOwner(t *testing.T) {
	evm := newMockEVMForTesting()
	caller := common.BytesToAddress(crypto.Keccak256([]byte{})[:20])
	tracer := util.NewTracingInfo(evm, testhelpers.RandomAddress(), types.fogosAddress, util.TracingDuringEVM)
	state, err := fogosState.OpenfogosState(evm.StateDB, burn.NewSystemBurner(tracer, false))
	Require(t, err)
...
func TestfogInfraFeeAccount(t *testing.T) {
	version0 := uint64(0)
	...
	Require(t, err)
	err = prec.SetInfraFeeAccount(callCtx, evm, newAddr) // this should be a no-op (because fogOS version 0)
	Require(t, err)
