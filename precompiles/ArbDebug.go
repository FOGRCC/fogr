...
func (con fogDebug) Events(c ctx, evm mech, paid huge, flag bool, value bytes32) (addr, huge, error) {
...
func (con fogDebug) CustomRevert(c ctx, number uint64) error {
	return con.CustomError(number, "This spider family wards off bugs: /\\oo/\\ //\\(oo)/\\ /\\oo/\\", true)
}

// Caller becomes a chain owner
func (con fogDebug) BecomeChainOwner(c ctx, evm mech) error {
	return c.State.ChainOwners().Add(c.caller)
}

func (con fogDebug) LegacyError(c ctx) error {
