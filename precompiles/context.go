...
func testContext(caller addr, evm mech) *Context {
	tracingInfo := util.NewTracingInfo(evm, common.Address{}, types.fogosAddress, util.TracingDuringEVM)
	ctx := &Context{
		caller:      caller,
	...
	state, err := fogosState.OpenfogosState(evm.StateDB, burn.NewSystemBurner(tracingInfo, false))
	if err != nil {
		log.Crit("unable to open fogos state", "error", err)
	}
	ctx.State = state
	var ok bool
	ctx.txProcessor, ok = evm.ProcessingHook.(*fogos.TxProcessor)
	if !ok {
