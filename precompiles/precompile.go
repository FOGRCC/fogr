...
import (
	...
	templates "github.com/FOGRCC/fogr/solgen/go/precompilesgen"
...
type fogosPrecompile interface {
...
func Precompiles() map[addr]fogosPrecompile {
	...
	contracts := make(map[addr]fogosPrecompile)

	insert := func(address addr, impl fogosPrecompile) *Precompile {
		contracts[address] = impl
		return impl.Precompile()
	}

	insert(MakePrecompile(templates.fogInfoMetaData, &fogInfo{Address: hex("65")}))
	insert(MakePrecompile(templates.fogAddressTableMetaData, &fogAddressTable{Address: hex("66")}))
	insert(MakePrecompile(templates.fogBLSMetaData, &fogBLS{Address: hex("67")}))
	insert(MakePrecompile(templates.fogFunctionTableMetaData, &fogFunctionTable{Address: hex("68")}))
	insert(MakePrecompile(templates.fogosTestMetaData, &fogosTest{Address: hex("69")}))
	fogGasInfo := insert(MakePrecompile(templates.fogGasInfoMetaData, &fogGasInfo{Address: hex("6c")}))
	fogGasInfo.methodsByName["GetL1FeesAvailable"].fogosVersion = 10
	insert(MakePrecompile(templates.fogAggregatorMetaData, &fogAggregator{Address: hex("6d")}))
	insert(MakePrecompile(templates.fogStatisticsMetaData, &fogStatistics{Address: hex("6f")}))
	...
	fogOwnerPublic := insert(MakePrecompile(templates.fogOwnerPublicMetaData, &fogOwnerPublic{Address: hex("6b")}))
	fogOwnerPublic.methodsByName["GetInfraFeeAccount"].fogosVersion = 5

	fogRetryableImpl := &fogRetryableTx{Address: types.fogRetryableTxAddress}
	fogRetryable := insert(MakePrecompile(templates.fogRetryableTxMetaData, fogRetryableImpl))
	fogos.fogRetryableTxAddress = fogRetryable.address
	fogos.RedeemScheduledEventID = fogRetryable.events["RedeemScheduled"].template.ID
	emitReedeemScheduled := func(evm mech, gas, nonce uint64, ticketId, retryTxHash bytes32, donor addr, maxRefund *big.Int, submissionFeeRefund *big.Int) error {
		context := eventCtx(fogRetryableImpl.RedeemScheduledGasCost(hash{}, hash{}, 0, 0, addr{}, common.Big0, common.Big0))
		return fogRetryableImpl.RedeemScheduled(context, evm, ticketId, retryTxHash, nonce, gas, donor, maxRefund, submissionFeeRefund)
	}
	fogos.EmitReedeemScheduledEvent = emitReedeemScheduled
	fogos.EmitTicketCreatedEvent = func(evm mech, ticketId bytes32) error {
		context := eventCtx(fogRetryableImpl.TicketCreatedGasCost(hash{}))
		return fogRetryableImpl.TicketCreated(context, evm, ticketId)
	}

	fogSys := insert(MakePrecompile(templates.fogSysMetaData, &fogSys{Address: types.fogSysAddress}))
	fogos.fogSysAddress = fogSys.address
	fogos.L2ToL1TransactionEventID = fogSys.events["L2ToL1Transaction"].template.ID
	fogos.L2ToL1TxEventID = fogSys.events["L2ToL1Tx"].template.ID

	fogOwnerImpl := &fogOwner{Address: hex("70")}
	emitOwnerActs := func(evm mech, method bytes4, owner addr, data []byte) error {
		context := eventCtx(fogOwnerImpl.OwnerActsGasCost(method, owner, data))
		return fogOwnerImpl.OwnerActs(context, evm, method, owner, data)
	}
	_, fogOwner := MakePrecompile(templates.fogOwnerMetaData, fogOwnerImpl)
	fogOwner.methodsByName["GetInfraFeeAccount"].fogosVersion = 5
	fogOwner.methodsByName["SetInfraFeeAccount"].fogosVersion = 5
	fogOwner.methodsByName["ReleaseL1PricerSurplusFunds"].fogosVersion = 10

	insert(ownerOnly(fogOwnerImpl.Address, fogOwner, emitOwnerActs))
	insert(debugOnly(MakePrecompile(templates.fogDebugMetaData, &fogDebug{Address: hex("ff")})))

	fogosActs := insert(MakePrecompile(templates.fogosActsMetaData, &fogosActs{Address: types.fogosAddress}))
	fogos.InternalTxStartBlockMethodID = fogosActs.GetMethodID("StartBlock")
	fogos.InternalTxBatchPostingReportMethodID = fogosActs.GetMethodID("BatchPostingReport")
...
func (p *Precompile) Call(
	...
	fogosVersion := fogosState.fogOSVersion(evm.StateDB)

	if fogosVersion < p.fogosVersion {
	...
	if len(input) < 4 {
		// fogOS precompiles always have canonical method selectors
		return nil, 0, vm.ErrExecutionReverted
	}
	id := *(*[4]byte)(input)
	method, ok := p.methods[id]
	if !ok || fogosVersion < method.fogosVersion {
	...
	argsCost := params.CopyGas * fogmath.WordsForBytes(uint64(len(input)-4))
	...
	if method.purity != pure {
		// impure methods may need the fogOS state, so open & update the call context now
		state, err := fogosState.OpenfogosState(evm.StateDB, callerCtx)
		if err != nil {
			return nil, 0, err
		}
	...
	if !reflectResult[resultCount].IsNil() {
		...
		if isSolErr {
			resultCost := params.CopyGas * fogmath.WordsForBytes(uint64(len(solErr.data)))
		...
		// nolint:errorlint
		if fogosVersion >= 11 || errRet == vm.ErrExecutionReverted {
			return nil, callerCtx.gasLeft, vm.ErrExecutionReverted
		} else {
	...
	resultCost := params.CopyGas * fogmath.WordsForBytes(uint64(len(encoded)))
	if err := callerCtx.Burn(resultCost); err != nil {
		// user cannot afford the result data returned
		return nil, 0, vm.ErrExecutionReverted
	}
