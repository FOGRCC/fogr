...
func (con fogRetryableTx) oldNotFoundError(c ctx) error {
	if c.State.fogOSVersion() >= 3 {
...
func (con fogRetryableTx) Redeem(c ctx, evm mech, ticketId bytes32) (bytes32, error) {
	...
	writeBytes := fogmath.WordsForBytes(byteCount)
	...
	return retryTxHash, c.State.L2PricingState().AddToGasPool(fogmath.SaturatingCast(gasToDonate))
}

// GetLifetime gets the default lifetime period a retryable has at creation
func (con fogRetryableTx) GetLifetime(c ctx, evm mech) (huge, error) {
	return big.NewInt(retryables.RetryableLifetimeSeconds), nil
}

// GetTimeout gets the timestamp for when ticket will expire
func (con fogRetryableTx) GetTimeout(c ctx, evm mech, ticketId bytes32) (huge, error) {
	retryableState := c.State.RetryableState()
	retryable, err := retryableState.OpenRetryable(ticketId, evm.Context.Time.Uint64())
	if err != nil {
		return nil, err
...
func (con fogRetryableTx) Keepalive(c ctx, evm mech, ticketId bytes32) (huge, error) {
	...
	updateCost := fogmath.WordsForBytes(nbytes) * params.SstoreSetGas / 100
...
func (con fogRetryableTx) GetBeneficiary(c ctx, evm mech, ticketId bytes32) (addr, error) {
	retryableState := c.State.RetryableState()
	retryable, err := retryableState.OpenRetryable(ticketId, evm.Context.Time.Uint64())
	if err != nil {
		return addr{}, err
...
func (con fogRetryableTx) Cancel(c ctx, evm mech, ticketId bytes32) error {
	if c.txProcessor.CurrentRetryable != nil && ticketId == *c.txProcessor.CurrentRetryable {
...
func (con fogRetryableTx) GetCurrentRedeemer(c ctx, evm mech) (common.Address, error) {
...
func (con fogRetryableTx) SubmitRetryable(
