...
const (
	...
	maxRequestedSeqNumOffset = fogutil.MessageIndex(10_000)
)

var (
	confirmedSequenceNumberGauge = metrics.NewRegisteredGauge("fogr/sequencenumber/confirmed", nil)
	cachedMessagesSentHistogram  = metrics.NewRegisteredHistogram("fogr/feed/clients/cache/sent", nil, metrics.NewBoundedHistogramSample())
)
...
func (b *SequenceNumberCatchupBuffer) getCacheMessages(requestedSeqNum fogutil.MessageIndex) *BroadcastMessage {
	if len(b.messages) == 0 {
		return nil
	}
	...
	if firstCachedSeqNum < requestedSeqNum {
		lastCachedSeqNum := firstCachedSeqNum + fogutil.MessageIndex(len(b.messages)-1)
...
func (b *SequenceNumberCatchupBuffer) deleteConfirmed(confirmedSequenceNumber fogutil.MessageIndex) {
	if len(b.messages) == 0 {
		return
	}
