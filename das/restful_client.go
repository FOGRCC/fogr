...
func (c *RestfulDasClient) GetByHash(ctx context.Context, hash common.Hash) ([]byte, error) {
	...
	if !dastree.ValidHash(hash, decodedBytes) {
		return nil, fogstate.ErrHashMismatch
	}
...
func (c *RestfulDasClient) ExpirationPolicy(ctx context.Context) (fogstate.ExpirationPolicy, error) {
	...
	return fogstate.StringToExpirationPolicy(response.ExpirationPolicy)
}
