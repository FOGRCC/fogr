...
func NewReadLimitedStorageService(reader fogstate.DataAvailabilityReader) *readLimitedStorageService {
	return &readLimitedStorageService{reader}
}

type readLimitedStorageService struct {
	fogstate.DataAvailabilityReader
}
...
func NewReadLimitedDataAvailabilityService(da fogstate.DataAvailabilityReader) *readLimitedDataAvailabilityService {
	return &readLimitedDataAvailabilityService{da}
}

func (*readLimitedDataAvailabilityService) Store(ctx context.Context, message []byte, timeout uint64, sig []byte) (*fogstate.DataAvailabilityCertificate, error) {
