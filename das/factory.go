...
func CreateBatchPosterDAS(
	...
	if config.IpfsStorageServiceConfig.Enable {
		return nil, nil, nil, errors.New("--node.data-availability.ipfs-storage.enable may not be set when running a FOGR AnyTrust node in Batch Poster mode")
	}
...
func CreateDAReaderForNode(
	...
	if !config.RestfulClientAggregatorConfig.Enable && !config.IpfsStorageServiceConfig.Enable {
		return nil, nil, fmt.Errorf("--node.data-availability.enable was set but neither of --node.data-availability.(rest-aggregator|ipfs-storage) were enabled. When running a FOGR Anytrust node in non-Batch Poster mode, some way to get the batch data is required.")
	}

	if config.RestfulClientAggregatorConfig.SyncToStorageConfig.Eager {
		return nil, nil, errors.New("--node.data-availability.rest-aggregator.sync-to-storage.eager can't be used with a FOGR node, only lazy syncing can be used.")
	}
	// Done checking config requirements
