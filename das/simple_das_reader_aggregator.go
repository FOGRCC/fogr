...
func NewRestfulClientAggregator(ctx context.Context, config *RestfulClientAggregatorConfig) (*SimpleDASReaderAggregator, error) {
	a := SimpleDASReaderAggregator{
		config: config,
		stats:  make(map[fogstate.DataAvailabilityReader]readerStats),
	}
...
type readerStatMessage struct {
	readerStat
	reader fogstate.DataAvailabilityReader
}
	...
	go func() {
		...
		for readers := si.nextReaders(); len(readers) != 0 && subCtx.Err() == nil; readers = si.nextReaders() {
			...
			for _, reader := range readers {
				wg.Add(1)
				go func(reader fogstate.DataAvailabilityReader) {
					defer wg.Done()
...
func (a *SimpleDASReaderAggregator) tryGetByHash(
	ctx context.Context, hash common.Hash, reader fogstate.DataAvailabilityReader,
) ([]byte, error) {
...
func (a *SimpleDASReaderAggregator) Start(ctx context.Context) {
	...
	updateRestfulDasClients := func(urls []string) {
		...
		combinedReaders := make(map[fogstate.DataAvailabilityReader]bool)
		...
		a.readers = make([]fogstate.DataAvailabilityReader, 0, len(combinedUrls))
...
func (a *SimpleDASReaderAggregator) ExpirationPolicy(ctx context.Context) (fogstate.ExpirationPolicy, error) {
	a.readersMutex.RLock()
	defer a.readersMutex.RUnlock()
	if len(a.readers) == 0 {
	...
	for _, serv := range a.readers {
		...
		if ep != expectedExpirationPolicy {
			return fogstate.MixedTimeout, nil
		}
	}
