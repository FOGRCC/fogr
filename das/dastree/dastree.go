...
func RecordHash(record func(bytes32, []byte), preimage ...[]byte) bytes32 {
	...
	unrolled := fogmath.ConcatByteSlices(preimage...)
	if len(unrolled) == 0 {
		return fogmath.FlipBit(keccord(prepend(LeafByte, keccord([]byte{}).Bytes())), 0)
	}

	length := uint32(len(unrolled))
	leaves := []node{}
	for bin := uint32(0); bin < length; bin += BinSize {
		end := fogmath.MinInt(bin+BinSize, length)
	...
	for len(layer) > 1 {
		...
		for i := 0; i < prior-1; i += 2 {
			...
			dataUnder := fogmath.ConcatByteSlices(firstHash, otherHash, fogmath.Uint32ToBytes(sizeUnder))
			parent := node{
	...
	return fogmath.FlipBit(layer[0].hash, 0)
}
...
func FlatHashToTreeHash(flat bytes32) bytes32 {
	...
	return fogmath.FlipBit(crypto.Keccak256Hash(FlatHashToTreeLeaf(flat)), 0)
}
...
func Content(root bytes32, oracle func(bytes32) ([]byte, error)) ([]byte, error) {
	...
	for len(stack) > 0 {
		...
		case NodeByte:
			...
			power := uint32(fogmath.NextOrCurrentPowerOf2(uint64(count)))
