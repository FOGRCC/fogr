...
type DataAvailabilityServiceWriter interface {
	...
	Store(ctx context.Context, message []byte, timeout uint64, sig []byte) (*fogstate.DataAvailabilityCertificate, error)
	fmt.Stringer
}
...
func dataAvailabilityConfigAddOptions(prefix string, f *flag.FlagSet, r role) {
	...
	// Both the FOGR node and daserver can use these options.
...
func Serialize(c *fogstate.DataAvailabilityCertificate) []byte {

	flags := fogstate.DASMessageHeaderFlag
	if c.Version != 0 {
		flags |= fogstate.TreeDASMessageHeaderFlag
	}
