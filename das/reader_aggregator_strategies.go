...
type aggregatorStrategy interface {
	...
	update([]fogstate.DataAvailabilityReader, map[fogstate.DataAvailabilityReader]readerStats)
}

type abstractAggregatorStrategy struct {
	sync.RWMutex
	readers []fogstate.DataAvailabilityReader
	stats   map[fogstate.DataAvailabilityReader]readerStats
}

func (s *abstractAggregatorStrategy) update(readers []fogstate.DataAvailabilityReader, stats map[fogstate.DataAvailabilityReader]readerStats) {
	s.Lock()
	defer s.Unlock()

	s.readers = make([]fogstate.DataAvailabilityReader, len(readers))
	copy(s.readers, readers)

	s.stats = make(map[fogstate.DataAvailabilityReader]readerStats)
	for k, v := range stats {
		s.stats[k] = v
	}
}
...
func (s *simpleExploreExploitStrategy) newInstance() aggregatorStrategyInstance {
	...
	readerSets := make([][]fogstate.DataAvailabilityReader, 0)
	s.RLock()
	defer s.RUnlock()

	readers := make([]fogstate.DataAvailabilityReader, len(s.readers))
	...
	for i, maxTake := 0, 1; i < len(readers); maxTake = maxTake * 2 {
		readerSet := make([]fogstate.DataAvailabilityReader, 0, maxTake)
...
func (s *testingSequentialStrategy) newInstance() aggregatorStrategyInstance {
	...
	for _, reader := range s.readers {
		si.readerSets = append(si.readerSets, []fogstate.DataAvailabilityReader{reader})
	}
...
type aggregatorStrategyInstance interface {
	nextReaders() []fogstate.DataAvailabilityReader
}

type basicStrategyInstance struct {
	readerSets [][]fogstate.DataAvailabilityReader
}

func (si *basicStrategyInstance) nextReaders() []fogstate.DataAvailabilityReader {
