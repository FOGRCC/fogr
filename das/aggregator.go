...
func (a *Aggregator) Store(ctx context.Context, message []byte, timeout uint64, sig []byte) (*fogstate.DataAvailabilityCertificate, error) {
	...
	for _, d := range a.services {
		go func(ctx context.Context, d ServiceDetails) {
			...
			const metricBase string = "fogr/das/rpc/aggregator/store"
	...
	var aggCert fogstate.DataAvailabilityCertificate
