...
type l1SyncService struct {
	...
	dataSource fogstate.DataAvailabilityReader
...
func newl1SyncService(config *SyncToStorageConfig, syncTo StorageService, dataSource fogstate.DataAvailabilityReader, l1Reader *headerreader.HeaderReader, inboxAddr common.Address) (*l1SyncService, error) {
...
func (s *l1SyncService) processBatchDelivered(ctx context.Context, batchDeliveredLog types.Log) error {
	...
	storeUntil := fogmath.SaturatingUAdd(deliveredEvent.TimeBounds.MaxTimestamp, uint64(s.config.RetentionPeriod.Seconds()))
	...
	if _, err = fogstate.RecoverPayloadFromDasBatch(ctx, deliveredEvent.BatchSequenceNumber.Uint64(), data, s.dataSource, preimages, fogstate.KeysetValidate); err != nil {
...
func FindDASDataFromLog(
	...
	if deliveredEvent.DataLocation == uint8(batchDataSeparateEvent) {
		...
		txData, err := fogutil.GetLogEmitterTxData(ctx, l1Client, batchDeliveredLog)
		if err != nil {
			return nil, err
		}
	...
	if !fogstate.IsDASMessageHeaderByte(data[0]) {
...
func NewSyncingFallbackStorageService(ctx context.Context,
	...
	backup fogstate.DataAvailabilityReader,
