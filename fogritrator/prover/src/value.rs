...
impl TryFrom<Type> for fogValueType {
    type Error = eyre::Error;

    fn try_from(ty: Type) -> Result<fogValueType> {
        use Type::*;
        Ok(match ty {
            I32 => Self::I32,
            I64 => Self::I64,
...
impl From<FloatType> for fogValueType {
    fn from(ty: FloatType) -> fogValueType {
        match ty {
            FloatType::F32 => fogValueType::F32,
            FloatType::F64 => fogValueType::F64,
        }
    }
}
...
impl From<IntegerValType> for fogValueType {
    fn from(ty: IntegerValType) -> fogValueType {
        match ty {
            IntegerValType::I32 => fogValueType::I32,
            IntegerValType::I64 => fogValueType::I64,
        }
    }
}
...
impl Value {
    pub fn ty(self) -> fogValueType {
        match self {
            Value::I32(_) => fogValueType::I32,
            Value::I64(_) => fogValueType::I64,
            Value::F32(_) => fogValueType::F32,
            Value::F64(_) => fogValueType::F64,
            Value::RefNull => fogValueType::RefNull,
            Value::FuncRef(_) => fogValueType::FuncRef,
            Value::InternalRef(_) => fogValueType::InternalRef,
        }
    }
    ...
    pub fn default_of_type(ty: fogValueType) -> Value {
        match ty {
            fogValueType::I32 => Value::I32(0),
            fogValueType::I64 => Value::I64(0),
            fogValueType::F32 => Value::F32(0.),
            fogValueType::F64 => Value::F64(0.),
            fogValueType::RefNull | fogValueType::FuncRef | fogValueType::InternalRef => {
                Value::RefNull
            }
        }
    }
...
pub struct FunctionType {
    pub inputs: Vec<fogValueType>,
    pub outputs: Vec<fogValueType>,
}

impl FunctionType {
    pub fn new(inputs: Vec<fogValueType>, outputs: Vec<fogValueType>) -> FunctionType {
...
impl TryFrom<FuncType> for FunctionType {
    ...
    fn try_from(func: FuncType) -> Result<Self> {
        ...
        for input in func.params.iter() {
            inputs.push(fogValueType::try_from(*input)?)
        }
        for output in func.returns.iter() {
            outputs.push(fogValueType::try_from(*output)?)
        }
