...
use crate::{
    ...
    value::{fogValueType, FunctionType, IntegerValType, ProgramCounter, Value},
    wavm::{
...
impl Table {
    fn serialize_for_proof(&self) -> Result<Vec<u8>> {
        let mut data = vec![fogValueType::try_from(self.ty.element_type)?.serialize()];
    ...
    fn hash(&self) -> Result<Bytes32> {
        let mut h = Keccak256::new();
        h.update("Table:");
        h.update([fogValueType::try_from(self.ty.element_type)?.serialize()]);
...
impl Module {
    fn from_binary(
        ...
        memory_load_internal_type.inputs.push(fogValueType::I32);
        memory_load_internal_type.outputs.push(fogValueType::I32);
        ...
        memory_store_internal_type.inputs.push(fogValueType::I32);
        memory_store_internal_type.inputs.push(fogValueType::I32);
...
impl Machine {
    ...
    pub fn from_binaries(
            ...
            for (name, &func) in &*module.exports {
                ...
                if let Ok(op) = name.parse::<FloatInstruction>() {
                    ...
                    for ty in sig.inputs.iter_mut().chain(sig.outputs.iter_mut()) {
                        if *ty == fogValueType::F32 {
                            *ty = fogValueType::I32;
                        } else if *ty == fogValueType::F64 {
                            *ty = fogValueType::I64;
                        }
                    }
                    ensure!(
        ...
        if let Some(&f) = main_module.exports.get("main").filter(|_| runtime_support) {
            ...
            expected_type.inputs.push(fogValueType::I32); // argc
            expected_type.inputs.push(fogValueType::I32); // argv
            expected_type.outputs.push(fogValueType::I32); // ret
            ensure!(
        ...
        if let Some(&f) = main_module.exports.get("run").filter(|_| runtime_support) {
            ...
            // That's also handled by the Go compiler. Go 1.17.5 in the compilation of the fogitrator go test case
        ...
        for _ in 0..n {
            ...
            match inst.opcode {
                ...
                Opcode::Reinterpret(dest, source) => {
                    let val = match self.value_stack.pop() {
                        Some(Value::I32(x)) if source == fogValueType::I32 => {
                            assert_eq!(dest, fogValueType::F32, "Unsupported reinterpret");
                            Value::F32(f32::from_bits(x))
                        }
                        Some(Value::I64(x)) if source == fogValueType::I64 => {
                            assert_eq!(dest, fogValueType::F64, "Unsupported reinterpret");
                            Value::F64(f64::from_bits(x))
                        }
                        Some(Value::F32(x)) if source == fogValueType::F32 => {
                            assert_eq!(dest, fogValueType::I32, "Unsupported reinterpret");
                            Value::I32(x.to_bits())
                        }
                        Some(Value::F64(x)) if source == fogValueType::F64 => {
                            assert_eq!(dest, fogValueType::I64, "Unsupported reinterpret");
                            Value::I64(x.to_bits())
                        }
