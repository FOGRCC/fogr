...
use crate::{
    ...
    utils::Bytes32,
    value::{fogValueType, FunctionType, IntegerValType},
};
use digest::Digest;
...
pub enum Opcode {
    ...
    Reinterpret(fogValueType, fogValueType),
    ...
    InitFrame,
    /// Unconditional jump to an fogitrary point in code.
    fogitraryJump,
    /// Conditional jump to an fogitrary point in code.
    fogitraryJumpIf,
    ...
    pub fn repr(self) -> u16 {
        match self {
            ...
            Opcode::MemoryLoad { ty, bytes, signed } => match (ty, bytes, signed) {
                (fogValueType::I32, 4, false) => 0x28,
                (fogValueType::I64, 8, false) => 0x29,
                (fogValueType::F32, 4, false) => 0x2A,
                (fogValueType::F64, 8, false) => 0x2B,
                (fogValueType::I32, 1, true) => 0x2C,
                (fogValueType::I32, 1, false) => 0x2D,
                (fogValueType::I32, 2, true) => 0x2E,
                (fogValueType::I32, 2, false) => 0x2F,
                (fogValueType::I64, 1, true) => 0x30,
                (fogValueType::I64, 1, false) => 0x31,
                (fogValueType::I64, 2, true) => 0x32,
                (fogValueType::I64, 2, false) => 0x33,
                (fogValueType::I64, 4, true) => 0x34,
                (fogValueType::I64, 4, false) => 0x35,
                _ => panic!(
            ...
            Opcode::MemoryStore { ty, bytes } => match (ty, bytes) {
                (fogValueType::I32, 4) => 0x36,
                (fogValueType::I64, 8) => 0x37,
                (fogValueType::F32, 4) => 0x38,
                (fogValueType::F64, 8) => 0x39,
                (fogValueType::I32, 1) => 0x3A,
                (fogValueType::I32, 2) => 0x3B,
                (fogValueType::I64, 1) => 0x3C,
                (fogValueType::I64, 2) => 0x3D,
                (fogValueType::I64, 4) => 0x3E,
                _ => panic!(
            ...
            Opcode::Reinterpret(dest, source) => match (dest, source) {
                (fogValueType::I32, fogValueType::F32) => 0xBC,
                (fogValueType::I64, fogValueType::F64) => 0xBD,
                (fogValueType::F32, fogValueType::I32) => 0xBE,
                (fogValueType::F64, fogValueType::I64) => 0xBF,
            ...
            Opcode::fogitraryJump => 0x8003,
            Opcode::fogitraryJumpIf => 0x8004,
...
pub fn wasm_to_wavm<'a>(
    ...
    macro_rules! reinterpret {
        ($dest:ident, $source:ident) => {{
            let op = Opcode::Reinterpret(fogValueType::$dest, fogValueType::$source);
            out.push(Instruction::simple(op));
        }};
    }
    macro_rules! float {
        ...
        (@impl $func:expr) => {{
            ...
            for &arg in sig.inputs.iter().rev() {
                match arg {
                    fogValueType::I32 | fogValueType::I64 => {}
                    fogValueType::F32 => reinterpret!(I32, F32),
                    fogValueType::F64 => reinterpret!(I64, F64),
            ...
            match outputs.as_slice() {
                &[fogValueType::I32] => {}
                &[fogValueType::I64] => {}
                &[fogValueType::F32] => reinterpret!(F32, I32),
                &[fogValueType::F64] => reinterpret!(F64, I64),
    ...
    macro_rules! branch {
        ($kind:ident, $depth:expr) => {{
            ...
            if jump_op == Opcode::fogitraryJumpIf {
                stack -= 1;
            } else {
                assert_eq!(jump_op, Opcode::fogitraryJump, "unrecognized jump op");
            }
            ...
            } else if jump_op == Opcode::fogitraryJump {
    ...
    for op in code {
        ...
        match op {
            ...
            Br { relative_depth } => branch!(fogitraryJump, *relative_depth),
            BrIf { relative_depth } => branch!(fogitraryJumpIf, *relative_depth),
            BrTable { table } => {
                ...
                for (index, target) in table.targets().enumerate() {
                    ...
                    opcode!(fogitraryJumpIf, @pop 1);
                }

                // nothing matched: drop the index and jump to the default.
                opcode!(Drop, @pop 1);
                branch!(fogitraryJump, table.default());
                ...
                for (jump, branch) in subjumps {
                    ...
                    stack = start_stack;
                    opcode!(Drop, @pop 1);
                    branch!(fogitraryJump, branch);
                }
            }
            Return => branch!(fogitraryJump, scopes.len() - 1),
            Call { function_index } => {
