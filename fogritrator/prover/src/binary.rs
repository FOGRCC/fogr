...
use crate::value::{fogValueType, FunctionType, IntegerValType, Value as LirValue};
    ...
    pub fn signature(&self) -> FunctionType {
        match *self {
            ...
            FloatInstruction::RelOp(t, _) => {
                FunctionType::new(vec![t.into(); 2], vec![fogValueType::I32])
            }
            ...
            FloatInstruction::F32DemoteF64 => {
                FunctionType::new(vec![fogValueType::F64], vec![fogValueType::F32])
            }
            FloatInstruction::F64PromoteF32 => {
                FunctionType::new(vec![fogValueType::F32], vec![fogValueType::F64])
            }
        }
    }
}
